...
# Full reporting protocol for the tiered GT4/GT5 weak-positive rescue run:
# train with official_best, sweep official_best.pt and best.pt on official-val,
# then run reporting-only TEST ACC and raw-Q12 diagnostics from each val-selected decode.
# TEST outputs are not selection inputs.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export RUN_NAME="${RUN_NAME:-query_alpha05_gt4gt5_tiered_geom_gt4pv12_env30_nocount_full_protocol_v1}"
