...
def check_context_and_index() -> None:
    ...
    require(manual.startswith("# Implementation Manual"), "implementation manual must describe the k56 branch implementation rules")
    require("5-25-3 algorithm body" in manual and "Agent Tooling" in manual, "implementation manual must document branch scope and agent tooling")
...
def check_project_contract_paths() -> None:
    required = (
        ...
        "tools/check_gcs_label_order_split.py",
    ...
    require("tools/check_model.py" in commands and "--cfg" in commands, "commands.md must call check_model.py with --cfg")
    require("check_model.py --model " not in commands, "commands.md must not use the nonexistent check_model --model flag")
    require("sweep_tusimple_official.py" in commands, "commands.md must document the branch official-val sweep helper")
    require("eval_tusimple_official.py" in commands, "commands.md must document the branch official eval helper")
