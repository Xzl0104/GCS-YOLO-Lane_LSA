...
# Override variables from the shell when needed, e.g. BATCH=16 DEVICE=1 bash scripts/run_gt5_short_geom_full_protocol_v1.sh

RUN_NAME="${RUN_NAME:-query_alpha05_gt5short_geom_w2_v1}"
...
# Default official-best hook and post-train sweeps both use a 2520-row query grid.
# Override OFFICIAL_* from the shell if a lighter training-time sweep is needed.
OFFICIAL_CONFS="${OFFICIAL_CONFS:-0.003 0.005 0.008 0.01 0.015 0.02}"
OFFICIAL_POINT_VALID_THRS="${OFFICIAL_POINT_VALID_THRS:-0.45 0.475 0.5 0.525 0.55 0.575 0.6}"
OFFICIAL_NMS_DIST_PXS="${OFFICIAL_NMS_DIST_PXS:-0 18 30 50 70}"
...
if best.get("decode_mode", "query") == "ordered_slot":
    raise SystemExit("This protocol script is for the query G1 run, not ordered_slot decode.")
