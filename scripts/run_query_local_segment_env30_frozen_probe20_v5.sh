...
SHORT_SEGMENT_BASE_CHOICE_WEIGHT="${SHORT_SEGMENT_BASE_CHOICE_WEIGHT:-0.0}"
...
SHORT_SEGMENT_UNIFIED_CHOICE_WEIGHT="${SHORT_SEGMENT_UNIFIED_CHOICE_WEIGHT:-0.0}"
SHORT_SEGMENT_UNIFIED_CHOICE_TEMPERATURE="${SHORT_SEGMENT_UNIFIED_CHOICE_TEMPERATURE:-0.25}"
SHORT_SEGMENT_UNIFIED_CHOICE_BASE_NEG_WEIGHT="${SHORT_SEGMENT_UNIFIED_CHOICE_BASE_NEG_WEIGHT:-0.25}"
SHORT_SEGMENT_CANDIDATE_AWARE_ASSIGNMENT="${SHORT_SEGMENT_CANDIDATE_AWARE_ASSIGNMENT:-0}"
...
SHORT_SEGMENT_MATCHED_ASSIGNMENT="${SHORT_SEGMENT_MATCHED_ASSIGNMENT:-0}"
SHORT_SEGMENT_OFFICIAL_QUALITY_TARGET="${SHORT_SEGMENT_OFFICIAL_QUALITY_TARGET:-0}"
SHORT_SEGMENT_OFFICIAL_PT_THRESH="${SHORT_SEGMENT_OFFICIAL_PT_THRESH:-0.85}"
SHORT_SEGMENT_BASE_VALID_THR="${SHORT_SEGMENT_BASE_VALID_THR:-0.6}"
SHORT_SEGMENT_BASE_CHOICE_ALL_QUERIES="${SHORT_SEGMENT_BASE_CHOICE_ALL_QUERIES:-0}"
SHORT_SEGMENT_BASE_CHOICE_NEG_WEIGHT="${SHORT_SEGMENT_BASE_CHOICE_NEG_WEIGHT:-0.25}"
...
MATCHED_ASSIGNMENT_ARGS=()
if is_true "${SHORT_SEGMENT_MATCHED_ASSIGNMENT}"; then
  MATCHED_ASSIGNMENT_ARGS=(--gcs-short-segment-matched-assignment)
fi
CANDIDATE_AWARE_ASSIGNMENT_ARGS=()
if is_true "${SHORT_SEGMENT_CANDIDATE_AWARE_ASSIGNMENT}"; then
  CANDIDATE_AWARE_ASSIGNMENT_ARGS=(--gcs-short-segment-candidate-aware-assignment)
fi
OFFICIAL_QUALITY_ARGS=()
if is_true "${SHORT_SEGMENT_OFFICIAL_QUALITY_TARGET}"; then
  OFFICIAL_QUALITY_ARGS=(--gcs-short-segment-official-quality-target)
fi
BASE_CHOICE_ALL_QUERIES_ARGS=()
if is_true "${SHORT_SEGMENT_BASE_CHOICE_ALL_QUERIES}"; then
  BASE_CHOICE_ALL_QUERIES_ARGS=(--gcs-short-segment-base-choice-all-queries)
fi
...
if is_true "${RUN_TRAIN}"; then
  ...
  python tools/train_gcs.py \
    ...
    --gcs-short-segment-base-choice-weight "${SHORT_SEGMENT_BASE_CHOICE_WEIGHT}" \
    ...
    --gcs-short-segment-unified-choice-weight "${SHORT_SEGMENT_UNIFIED_CHOICE_WEIGHT}" \
    --gcs-short-segment-unified-choice-temperature "${SHORT_SEGMENT_UNIFIED_CHOICE_TEMPERATURE}" \
    --gcs-short-segment-unified-choice-base-neg-weight "${SHORT_SEGMENT_UNIFIED_CHOICE_BASE_NEG_WEIGHT}" \
    "${CANDIDATE_AWARE_ASSIGNMENT_ARGS[@]}" \
    "${LISTWISE_SCOPE_ARGS[@]}" \
    "${BASE_PRESERVE_ARGS[@]}" \
    "${MATCHED_ASSIGNMENT_ARGS[@]}" \
    "${OFFICIAL_QUALITY_ARGS[@]}" \
    --gcs-short-segment-official-pt-thresh "${SHORT_SEGMENT_OFFICIAL_PT_THRESH}" \
    --gcs-short-segment-base-valid-thr "${SHORT_SEGMENT_BASE_VALID_THR}" \
    "${BASE_CHOICE_ALL_QUERIES_ARGS[@]}" \
    --gcs-short-segment-base-choice-neg-weight "${SHORT_SEGMENT_BASE_CHOICE_NEG_WEIGHT}" \
