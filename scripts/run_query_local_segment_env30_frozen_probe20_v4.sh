...
# Frozen-env30 local short-segment proposal probe: load env30 official_best.pt,
# freeze every non-short-segment parameter, train only the local segment head,
# verify base decode on official-val, run hard official-GT diagnostics, and keep
# TEST closed.

RUN_NAME="${RUN_NAME:-query_local_segment_env30_frozen_probe20_v4}"
PROJECT="${PROJECT:-runs/gcs_lane}"
MODEL="${MODEL:-ultralytics/cfg/models/gcs/gcs-yolo-lane-s-q12-k56-local-segment-proposal-v4.yaml}"
...
if is_true "${RUN_TESTS}"; then
  echo "RUN_TESTS must stay 0 for this local-segment probe. Select on official-val before any TEST run." >&2
  exit 2
fi
...
SEGMENT_WEIGHTS_FOR_DIAG="${SEGMENT_WEIGHTS_FOR_DIAG:-${LAST_WEIGHTS}}"
BASE_SWEEP_DIR="${BASE_SWEEP_DIR:-${PROJECT}/${RUN_NAME}_official_best_val_base_sweep}"
HARD_DIAG_VAL_DIR="${HARD_DIAG_VAL_DIR:-${PROJECT}/${RUN_NAME}_hard_segment_diag_val_last}"
HARD_DIAG_TRAIN0601_DIR="${HARD_DIAG_TRAIN0601_DIR:-${PROJECT}/${RUN_NAME}_hard_segment_diag_train0601_last}"
...
if [[ ! -f "${SEGMENT_WEIGHTS_FOR_DIAG}" ]]; then
  if [[ -f "${OFFICIAL_BEST_WEIGHTS}" ]]; then
    SEGMENT_WEIGHTS_FOR_DIAG="${OFFICIAL_BEST_WEIGHTS}"
  elif [[ -f "${BEST_WEIGHTS}" ]]; then
    SEGMENT_WEIGHTS_FOR_DIAG="${BEST_WEIGHTS}"
  else
    echo "Missing segment weights for diagnostics: ${SEGMENT_WEIGHTS_FOR_DIAG}" >&2
    exit 2
  fi
fi

echo "Base-decode sweep weights: ${BASE_WEIGHTS_FOR_SWEEP}"
echo "Segment diagnostics weights: ${SEGMENT_WEIGHTS_FOR_DIAG}"
...
if is_true "${RUN_HARD_DIAG}"; then
  python tools/diagnose_gcs_short_candidate_hard_coverage.py \
    --weights "${SEGMENT_WEIGHTS_FOR_DIAG}" \
    --archive-root "${ARCHIVE_ROOT}" \
    --split val \
    --gt-json "${GT_JSON}" \
    --imgsz 544 960 \
  ...
  python tools/diagnose_gcs_short_candidate_hard_coverage.py \
    ...
    --save-dir "${HARD_DIAG_TRAIN0601_DIR}"
fi

echo "Local short-segment probe complete. TEST remained closed."
