...
The active source/config is restored to commit `86c8fb31cb4b48a53086be183478a95b0807753d` (`Add GT4 GT5 weak geometry rescue run`), the env30 baseline. All commits after this env30 anchor are rejected experiment records only unless a future task explicitly reopens one with new official-val/train-side gates. Their code, configs, scripts, tools, reference banks, and documentation must not be treated as active behavior.

## Branch Scope

Do not silently import later mainline mechanisms into this branch. In particular, do not add later mainline Count Head, Quality Head, Survival Head, near-miss mining, or mainline K56 candidate scripts unless a future task explicitly asks for that algorithm change. The active env30 baseline includes the 2026-06-27 user-requested, default-off `count_boundary_loss`, the 2026-06-27 user-requested, default-off train-only `gcs_hard_sampling`, the branch-local default-off `gcs_spurious_neg` family recorded in `docs/agent-context/current-contracts.md`, training-time `official_best`, `valid_before_maxdet`, and the 2026-07-06 user-requested default-off query Count Head ablation enabled by `ultralytics/cfg/models/gcs/gcs-yolo-lane-s-q12-k56-count.yaml`. Post-`86c8fb31c` experiments, including env30 staticref/valid-neg/near20 follow-ups, Q20/Q24 protected-static or dual-head routes, Q12 dual-head, query extent, short local-refine, lateral candidate, and gated candidate mechanisms, are rejected legacy records only.
