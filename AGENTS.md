...
The branch keeps the historical 5-25-3 algorithm body, the TuSimple fixed-y contract, the explicit default-off `count_boundary_loss`, the explicit default-off train-only `gcs_hard_sampling`, the default-off `gcs_spurious_neg`, the training-time `official_best` protocol additions, the default-off `valid_before_maxdet` decode option requested on this branch, the 2026-07-06 user-requested default-off query Count Head ablation, the default-off env30 short-geometry/boundary-pseudo protocol available at commit `86c8fb31c`, and the 2026-07-20 user-requested default-off `gcs_boundary_pseudo_max_gt_count` upper-bound extension for GT3/GT4-only boundary-pseudo experiments.
