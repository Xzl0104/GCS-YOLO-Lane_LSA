...
formal batch: 32
workers: 4
GPU memory: about 18.6 GiB / 24.6 GiB on RTX 4090 24GB
GPU utilization sample at 2026-06-14 epoch101 audit: 35%, 75%, 15%, 94%, 100%, 18%, 62%, 82%, 17%, 39%; treat as point-in-time utilization showing the healthy batch=32 run is not obviously underusing the server GPU
status at 2026-06-14 epoch101 audit: training alive with ordinary-val row 102/180; root official_best_summary has official-val candidates through epoch 101
ordinary val latest row: epoch=102, val/f1=0.956120, precision=0.956857, recall=0.955385, val/FP=56, val/FN=58, val/decode/count_head_k=3.60881, val/decode/final_pred_lanes=3.57576, val/decode/k5_to_output4_rate=0.173913
ordinary val best row so far by val/f1: epoch=97, val/f1=0.959724, precision=0.957154, recall=0.962308
official_best so far: epoch 83, official_acc=0.956905, FP=0.042149, FN=0.031680
official_best count/GT5 diagnostics: count_acc_3/4/5=0.941704/0.909091/0.810811, gt5_output5_rate=0.810811, gt5_count_head_under_rate=0.094595, gt5_valid_points_fail_rate=0.108108, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, decode/k5_to_output4_rate=0.186667, rescue_precision=0.852459, rate_3_to_4=0.058296, rate_4_to_5=0.030303, rate_5_to_4=0.189189, matched/unmatched_quality_mean=0.874704/0.782380
official_top_k retained epochs and ACC: 83=0.956905, 97=0.956630, 101=0.956168, 95=0.956133, 96=0.955896
latest official-val candidate: epoch=101, official_acc=0.956168, FP=0.048990, FN=0.036501, count_acc_3/4/5=0.928251/0.893939/0.783784, gt5_output5_rate=0.783784, gt5_count_head_under_rate=0.108108, gt5_valid_points_fail_rate=0.108108, gt5_candidate_pool_shortfall_rate=0.013514, gt5_top5_suppressed_by_nms_rate=0.000000, rescue_precision=0.800000
diagnostic top-k notes: epoch83 remains K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.003149, countboundary 0.954137 by +0.002768, and old FT6 0.954782 by +0.002123. It remains below legacy 0.959224 by -0.002319 and below the 0.97 objective. Epoch101 entered Top-K but did not beat epoch83; epoch97 is also now the ordinary-val best row and the second official Top-K candidate. This confirms official-val is still moving, so the run is not a full-run plateau. Candidate supply and GT5 NMS are non-dominant; the next K56 candidate, if needed after clearer plateau or completion, should preserve epoch83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 102 rows; 102 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch101. A text-artifact scan of 205 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], and gcs_imgsz=[544, 960]. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
