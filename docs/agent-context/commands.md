...
GPU utilization samples at 2026-06-14 epoch145/146 audits ranged from low/bursty to about 70%; `ps`, `results.csv`, and checkpoint updates confirm the healthy batch=32 run is still alive, so do not alter batch mid-run
status at 2026-06-14 epoch145/146 audit: training alive with ordinary-val row 146/180; root official_best_summary has official-val candidates through epoch 145
ordinary val latest row: epoch=146, val/f1=0.958621, precision=0.954962, recall=0.962308, fp=59, fn=49, val/decode/k5_to_output4_rate=0.081081
...
official_top_k retained epochs and ACC: 140=0.959200, 141=0.959131, 143=0.959116, 142=0.959034, 144=0.958994
latest official-val candidate: epoch=145, official_acc=0.958931, FP=0.046373, FN=0.028237, count_acc_3/4/5=0.919283/0.863636/0.837838, gt5_output5_rate=0.837838, gt5_count_head_under_rate=0.067568, gt5_valid_points_fail_rate=0.094595, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, decode/k5_to_output4_rate=0.105263, rescue_precision=0.764706, matched/unmatched_quality_mean=0.910776/0.826427
diagnostic top-k notes: epoch140 remains K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.005444, countboundary 0.954137 by +0.005063, old FT6 0.954782 by +0.004418, prior K56 epoch127 best 0.958484 by +0.000716, and epoch115 by +0.001240. It remains below legacy 0.959224 by -0.000024 and below the 0.97 objective. Epoch142 is now the best ordinary-val row, and epoch145 is close enough to be diagnostically useful while improving GT5 output/count5 retention (`gt5_output5_rate=0.837838`, `count_acc_5=0.837838`) at a small ACC cost. Candidate supply remains healthy; GT5 NMS is low. The next K56 candidate, if needed after clearer plateau or completion, should preserve epoch140 FP/FN gains while approaching epoch145 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 146 rows; 147 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch145. A text-artifact scan of 295 run files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `Traceback`, `RuntimeError`, `shape error`, or `shape mismatch` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
