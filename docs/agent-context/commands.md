...
GPU utilization sample at 2026-06-14 epoch120/121 audit: 17%, 100%, 100%, 17%, 63%; `ps` confirms the healthy batch=32 run is still alive, so do not alter batch mid-run
status at 2026-06-14 epoch120/121 audit: training alive with ordinary-val row 121/180; root official_best_summary has official-val candidates through epoch 120
ordinary val latest row: epoch=121, val/f1=0.957790, precision=0.955590, recall=0.960000, val/decode/k5_to_output4_rate=0.094595
...
official_top_k retained epochs and ACC: 115=0.957960, 114=0.957693, 109=0.957664, 110=0.957280, 113=0.957274
latest official-val candidate: epoch=120, official_acc=0.957116, FP=0.049311, FN=0.035583, count_acc_3/4/5=0.932735/0.863636/0.837838, gt5_output5_rate=0.837838, gt5_count_head_under_rate=0.094595, gt5_valid_points_fail_rate=0.081081, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, decode/k5_to_output4_rate=0.109589, rescue_precision=0.784615
diagnostic top-k notes: epoch115 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.004204, countboundary 0.954137 by +0.003823, old FT6 0.954782 by +0.003178, prior K56 epoch83 best 0.956905 by +0.001055, and epoch109 by +0.000296. It remains below legacy 0.959224 by -0.001264 and below the 0.97 objective. Epoch120 did not beat epoch115 official ACC, but it improved GT5 output and valid-points survival (`gt5_output5_rate=0.837838`, `gt5_valid_points_fail_rate=0.081081`, `decode/k5_to_output4_rate=0.109589`) while paying higher FP/FN. Candidate supply and GT5 NMS remain non-dominant. The next K56 candidate, if needed after clearer plateau or completion, should preserve epoch115 official ACC gain while approaching epoch120 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 121 rows; 121 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch120. A text-artifact scan of 243 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
