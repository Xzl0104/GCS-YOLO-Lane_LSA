...
GPU utilization sample at 2026-06-14 12:19 CST: 0%, 3%, 0%, 22%, 0%; treat as point-in-time utilization, not a reason to interrupt the healthy run
status at 2026-06-14 12:19 CST: training alive around epoch 88/180; results.csv has ordinary-val row 88, and official_best_summary has official-val candidates through epoch 87
ordinary val latest row: epoch=88, val/f1=0.955888, precision=0.953328, recall=0.958462, val/decode/count_head_k=3.64463, val/decode/final_pred_lanes=3.60055, val/decode/k5_to_output4_rate=0.207792
...
official_top_k retained epochs and ACC: 83=0.956905, 85=0.955739, 84=0.955589, 87=0.955453, 82=0.955292
latest official-val candidate: epoch=87, official_acc=0.955453, FP=0.049954, FN=0.034894
diagnostic top-k notes: epoch83 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.003149, countboundary 0.954137 by +0.002768, and old FT6 0.954782 by +0.002123. It remains below legacy 0.959224 by -0.002319 and below the 0.97 objective. Epoch83 improves FP/FN over epoch82 and gains +0.001613 official ACC, but GT5 output drops from epoch82's 0.891892 to 0.810811 and gt5_valid_points_fail_rate rises to 0.108108. The run has not clearly plateaued because official-best just refreshed and epochs 84/85/87 remain Top-K. Candidate supply and GT5 NMS are non-dominant; the next K56 candidate, if needed after plateau, should preserve epoch83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values; 88 run JSON files have no numeric NaN/Inf values; official-val sweeps exist through epoch87. A text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val and gcs_official_best_split=val.
decision: continue monitoring; K56 has beaten several active official-val references and just refreshed official-best, but remains below legacy 0.959224 and the 0.97 objective. Do not promote, do not use test, and do not launch a replacement experiment while the healthy baseline is still improving. Keep batch=32 for this in-progress run; before a future formal run, a throughput-only batch probe may be used to better utilize the RTX 4090 24GB server without changing the official-val protocol.
```
