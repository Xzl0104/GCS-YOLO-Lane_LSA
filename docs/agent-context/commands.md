...
formal batch: 32
workers: 4
GPU memory: about 17.9 GiB / 24.6 GiB on RTX 4090 24GB
GPU utilization sample at 2026-06-14 12:45 CST: 100%, 18%, 97%, 18%, 68%, 100%; treat as point-in-time utilization showing the healthy run has active training load
status at 2026-06-14 12:45 CST: training alive around epoch 95/180; results.csv has ordinary-val row 95, and official_best_summary has official-val candidates through epoch 94
ordinary val latest row: epoch=95, val/f1=0.955086, precision=0.953257, recall=0.956923, val/decode/count_head_k=3.63636, val/decode/final_pred_lanes=3.59504, val/decode/k5_to_output4_rate=0.194805
ordinary val best row so far by val/f1: epoch=73, val/f1=0.959693, precision=0.957854, recall=0.961538
...
official_top_k retained epochs and ACC: 83=0.956905, 85=0.955739, 84=0.955589, 87=0.955453, 94=0.955354
latest official-val candidate: epoch=94, official_acc=0.955354, FP=0.050918, FN=0.036272, count_acc_3/4/5=0.923767/0.893939/0.810811, gt5_output5_rate=0.810811, gt5_count_head_under_rate=0.094595, gt5_valid_points_fail_rate=0.094595, gt5_candidate_pool_shortfall_rate=0.013514, gt5_top5_suppressed_by_nms_rate=0.000000, rescue_precision=0.828125, matched/unmatched_quality_mean=0.882169/0.755599
diagnostic top-k notes: epoch83 remains K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.003149, countboundary 0.954137 by +0.002768, and old FT6 0.954782 by +0.002123. It remains below legacy 0.959224 by -0.002319 and below the 0.97 objective. Epoch94 entered Top-K but did not beat epoch83; it improves gt5_valid_points_fail_rate versus epoch83 (0.094595 vs 0.108108) while paying higher FP/FN and candidate shortfall. This suggests a short-term official-val plateau after epoch83, but not a full-run plateau because training is only around epoch95/180. Candidate supply and GT5 NMS are non-dominant; the next K56 candidate, if needed after clearer plateau or completion, should preserve epoch83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values; 95 run JSON files have no numeric NaN/Inf values; official-val sweeps exist through epoch94. A text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val, gcs_official_best_split=val, and imgsz=[544, 960].
