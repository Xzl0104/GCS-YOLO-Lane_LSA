...
GPU utilization sample at 2026-06-14 epoch116 audit: 17%; `ps` confirms the healthy batch=32 run is still alive, so do not alter batch mid-run
status at 2026-06-14 epoch116 audit: training alive with ordinary-val row 116/180; root official_best_summary has official-val candidates through epoch 116
ordinary val latest row: epoch=116, val/f1=0.956221, precision=0.954755, recall=0.957692, val/decode/count_head_k=3.61157, val/decode/final_pred_lanes=3.59229, val/decode/k5_to_output4_rate=0.097222
...
latest official-val candidate: epoch=116, official_acc=0.956564, FP=0.048990, FN=0.034665, count_acc_3/4/5=0.928251/0.848485/0.797297, gt5_output5_rate=0.797297, gt5_count_head_under_rate=0.108108, gt5_valid_points_fail_rate=0.108108, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, rescue_precision=0.746032
diagnostic top-k notes: epoch115 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.004204, countboundary 0.954137 by +0.003823, old FT6 0.954782 by +0.003178, prior K56 epoch83 best 0.956905 by +0.001055, and epoch109 by +0.000296. It remains below legacy 0.959224 by -0.001264 and below the 0.97 objective. The gain comes with lower FP/FN than epoch109, but GT5 output and valid-points survival are weaker than the best GT5-retention rows. Candidate supply and GT5 NMS remain non-dominant. The next K56 candidate, if needed after clearer plateau or completion, should preserve epoch115 official ACC gain while reducing GT5 5->4 failures through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 116 rows; 117 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch116. A text-artifact scan of 235 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
