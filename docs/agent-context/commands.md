...
GPU utilization sample at 2026-06-14 11:37 CST: point samples ranged from 0-22% to 57%; treat as point-in-time utilization, not a reason to interrupt the healthy run
status at 2026-06-14 11:37 CST: training alive; results.csv has ordinary-val row 75, and official_best_summary has official-val candidates through epoch 74
ordinary val latest row: epoch=75, val/f1=0.955684, precision=0.957529, recall=0.953846, val/decode/count_head_k=3.61984, val/decode/final_pred_lanes=3.56749, val/decode/k5_to_output4_rate=0.246753, val/decode/candidate_pool_shortfall_rate=0.002755, val/decode/top5_suppressed_by_nms_rate=0.063361
ordinary val best row so far by val/f1: epoch=73, val/f1=0.959693, precision=0.957854, recall=0.961538
official_best so far: epoch 73, official_acc=0.955238, FP=0.046465, FN=0.031910
official_best count/GT5 diagnostics: count_acc_3/4/5=0.937220/0.848485/0.810811, gt5_output5_rate=0.810811, gt5_count_head_under_rate=0.040541, gt5_valid_points_fail_rate=0.148649, gt5_candidate_pool_shortfall_rate=0.013514, gt5_top5_suppressed_by_nms_rate=0.013514, decode/k5_to_output4_rate=0.172840, rescue_precision=0.761194, rate_4_to_5=0.090909
official_top_k retained epochs and ACC: 73=0.955238, 61=0.954807, 72=0.954547, 69=0.954073, 68=0.953795
latest official-val candidate: epoch=74, official_acc=0.953752, FP=0.051653, FN=0.041552
diagnostic top-k notes: epoch73 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.001482, countboundary 0.954137 by +0.001101, and old FT6 0.954782 by +0.000456. It remains below legacy 0.959224 by -0.003986 and below the 0.97 objective. Epoch73 also has the best ordinary val/f1 so far, but selection remains official-val. Candidate supply and GT5 NMS remain non-dominant; the stable bottleneck is fifth-lane valid-point/output survival versus FP/GT4/GT5 count tradeoff, with gt5_valid_points_fail_rate still above the 0.1 diagnostic target and rate_4_to_5=0.090909.
errors: process is alive; results.csv has no numeric NaN/Inf values; 75 run JSON files have no numeric NaN/Inf values; official-val sweeps exist through epoch74. A text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val and gcs_official_best_split=val.
decision: continue monitoring; K56 has beaten several active official-val references and just refreshed official-best, but remains below legacy 0.959224 and still fails GT5 diagnostic targets. Do not promote, do not use test, and do not launch a replacement experiment while the healthy baseline is still improving. Keep batch=32 for this in-progress run; before a future formal run, a throughput-only batch probe may be used to better utilize the RTX 4090 24GB server without changing the official-val protocol.
```
