...
GPU utilization sample at 2026-06-14 epoch138 audit: latest sample 0%, prior burst samples 18%, 39%, 86%, 18%, 52%; `ps`, `results.csv`, and checkpoint updates confirm the healthy batch=32 run is still alive, so do not alter batch mid-run
status at 2026-06-14 epoch138 audit: training alive with ordinary-val row 138/180; root official_best_summary has official-val candidates through epoch 137
ordinary val latest row: epoch=138, val/f1=0.959020, precision=0.954996, recall=0.963077, fp=59, fn=48, val/decode/k5_to_output4_rate=0.094595
...
official_top_k retained epochs and ACC: 127=0.958484, 128=0.958443, 137=0.958429, 129=0.958332, 134=0.958103
latest official-val candidate: epoch=137, official_acc=0.958429, FP=0.048577, FN=0.031680, count_acc_3/4/5=0.923767/0.878788/0.770270, gt5_output5_rate=0.770270, gt5_count_head_under_rate=0.094595, gt5_valid_points_fail_rate=0.135135, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.013514, decode/k5_to_output4_rate=0.162162, rescue_precision=0.774194, matched/unmatched_quality_mean=0.909715/0.820663
diagnostic top-k notes: epoch127 remains K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.004728, countboundary 0.954137 by +0.004347, old FT6 0.954782 by +0.003702, prior K56 epoch83 best 0.956905 by +0.001579, and epoch115 by +0.000524. It remains below legacy 0.959224 by -0.000740 and below the 0.97 objective. Epoch128 is nearly tied with epoch127 while improving GT5 retention (`gt5_output5_rate=0.837838`, `gt5_valid_points_fail_rate=0.108108`), but neither exceeds legacy yet. Epoch137 entered Top-K third but regressed GT5 output/valid-points versus epoch128. Candidate supply remains healthy; GT5 NMS is low. The next K56 candidate, if needed after clearer plateau or completion, should preserve epoch127 FP/FN gains while approaching epoch128 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 138 rows; 138 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch137. A text-artifact scan of 277 run files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `Traceback`, `RuntimeError`, `shape error`, or `shape mismatch` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
