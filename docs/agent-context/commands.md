...
GPU utilization sample at 2026-06-14 12:06 CST: 10%, 19%, 3%, 0%, 0%; treat as point-in-time utilization, not a reason to interrupt the healthy run
status at 2026-06-14 12:06 CST: training alive; results.csv has ordinary-val row 83, and official_best_summary has official-val candidates through epoch 82
ordinary val latest row: epoch=83, val/f1=0.958093, precision=0.957725, recall=0.958462, val/decode/count_head_k=3.60881, val/decode/final_pred_lanes=3.58402
ordinary val best row so far by val/f1: epoch=73, val/f1=0.959693, precision=0.957854, recall=0.961538
official_best so far: epoch 82, official_acc=0.955292, FP=0.052525, FN=0.036042
official_best count/GT5 diagnostics: count_acc_3/4/5=0.919283/0.878788/0.891892, gt5_output5_rate=0.891892, gt5_count_head_under_rate=0.040541, gt5_valid_points_fail_rate=0.067568, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, decode/k5_to_output4_rate=0.135802, rescue_precision=0.814286, rate_4_to_5=0.060606, rate_5_to_4=0.108108, rate_3_to_4=0.080717
official_top_k retained epochs and ACC: 82=0.955292, 73=0.955238, 76=0.955010, 61=0.954807, 80=0.954728
latest official-val candidate: epoch=82, official_acc=0.955292, FP=0.052525, FN=0.036042
diagnostic top-k notes: epoch82 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.001536, countboundary 0.954137 by +0.001155, and old FT6 0.954782 by +0.000510. It remains below legacy 0.959224 by -0.003932 and below the 0.97 objective. Epoch82 improves GT5 survival over epoch73 but pays higher FP/FN than an ideal candidate; it is a real but tiny official-val gain over epoch73 (+0.000054). Because the latest best was just refreshed and epoch80 remains Top-K, this run has not clearly plateaued. Candidate supply and GT5 NMS are non-dominant; the next K56 candidate, if needed after plateau, should preserve epoch82 GT5 survival while reducing FP and GT3/GT4 over-count pressure.
errors: process is alive; results.csv has no numeric NaN/Inf values; 83 run JSON files have no numeric NaN/Inf values; official-val sweeps exist through epoch82. A text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val and gcs_official_best_split=val.
