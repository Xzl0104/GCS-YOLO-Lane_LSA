...
GPU utilization sample at 2026-06-14 epoch107 audit: 0-100%; treat as point-in-time utilization showing the healthy batch=32 run is not obviously underusing server VRAM
status at 2026-06-14 epoch107 audit: training alive with ordinary-val row 108/180; root official_best_summary has official-val candidates through epoch 107
ordinary val latest row: epoch=108, val/f1=0.956890, precision=0.957627, recall=0.956154, val/decode/count_head_k=3.61157, val/decode/final_pred_lanes=3.57576, val/decode/k5_to_output4_rate=0.180556
...
official_top_k retained epochs and ACC: 83=0.956905, 107=0.956640, 97=0.956630, 101=0.956168, 95=0.956133
latest official-val candidate: epoch=107, official_acc=0.956640, FP=0.042424, FN=0.031221, count_acc_3/4/5=0.937220/0.863636/0.743243, gt5_output5_rate=0.743243, gt5_count_head_under_rate=0.108108, gt5_valid_points_fail_rate=0.162162, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, rescue_precision=0.793103
diagnostic top-k notes: epoch83 remains K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.003149, countboundary 0.954137 by +0.002768, and old FT6 0.954782 by +0.002123. It remains below legacy 0.959224 by -0.002319 and below the 0.97 objective. Epoch107 entered Top-K second and is only 0.000265 below epoch83, with similar FP/FN but worse GT5 output and valid-points survival. This confirms official-val is still moving, so the run is not a full-run plateau. Candidate supply and GT5 NMS are non-dominant; the next K56 candidate, if needed after clearer plateau or completion, should preserve epoch83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 107 rows in the subagent audit; 107 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch107. A text-artifact scan of 215 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
