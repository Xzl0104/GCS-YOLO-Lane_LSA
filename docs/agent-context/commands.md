...
For the interval-5 version of the same full protocol, use:

```bash
bash scripts/run_gt5_short_geom_full_protocol_interval2_v1.sh
```

It keeps the same training recipe, but scans every 5 epochs, then runs the post-train official-val sweeps and final test reports for both `official_best.pt` and `best.pt`.
