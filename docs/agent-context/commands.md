...
GPU utilization sample at 2026-06-14 epoch126 audit: 2%, 0%, 0%, 0%, 18%, final sample 0%; `ps` and checkpoint updates confirm the healthy batch=32 run is still alive, so do not alter batch mid-run
status at 2026-06-14 epoch126 audit: training alive with ordinary-val row 126/180; root official_best_summary has official-val candidates through epoch 125
ordinary val latest row: epoch=126, val/f1=0.956956, precision=0.956221, recall=0.957692, fp=57, fn=55, val/decode/k5_to_output4_rate=0.138889
ordinary val best row so far by val/f1: epoch=97, val/f1=0.959724, precision=0.957154, recall=0.962308
official_best so far: epoch 115, official_acc=0.957960, FP=0.046006, FN=0.032599
official_best count/GT5 diagnostics: count_acc_3/4/5=0.928251/0.863636/0.702703, gt5_output5_rate=0.702703, gt5_count_head_under_rate=0.121622, gt5_valid_points_fail_rate=0.175676, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, decode/k5_to_output4_rate=0.211268, rescue_precision=0.785714, rate_3_to_4=0.071749, rate_4_to_5=0.060606, rate_5_to_4=0.297297, matched/unmatched_quality_mean=0.896183/0.816861
official_top_k retained epochs and ACC: 115=0.957960, 114=0.957693, 109=0.957664, 121=0.957589, 122=0.957344
latest official-val candidate: epoch=125, official_acc=0.957193, FP=0.048026, FN=0.033287, count_acc_3/4/5=0.923767/0.909091/0.783784, gt5_output5_rate=0.783784, gt5_count_head_under_rate=0.121622, gt5_valid_points_fail_rate=0.094595, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.000000, decode/k5_to_output4_rate=0.140845, rescue_precision=0.770492, matched/unmatched_quality_mean=0.903824/0.808772
diagnostic top-k notes: epoch115 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.004204, countboundary 0.954137 by +0.003823, old FT6 0.954782 by +0.003178, prior K56 epoch83 best 0.956905 by +0.001055, and epoch109 by +0.000296. It remains below legacy 0.959224 by -0.001264 and below the 0.97 objective. Later epochs 121/122 entered Top-K and improved GT5 retention versus epoch115 (`gt5_output5_rate=0.837838/0.824324`, `gt5_valid_points_fail_rate=0.081081/0.081081`) but still did not beat epoch115 official ACC because FP/FN and count-confusion tradeoffs rose. Epoch125 is slightly above epoch120 but remains below epoch115 by -0.000767. Candidate supply and GT5 NMS remain non-dominant. The next K56 candidate, if needed after clearer plateau or completion, should preserve epoch115 official ACC gain while approaching epoch121/122 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 126 rows; 126 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch125. A text-artifact scan of 253 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
decision: continue monitoring; K56 has beaten several active official-val references but remains below legacy 0.959224 and the 0.97 objective. Do not promote, do not use test, and do not launch a replacement Count/Quality calibration while the first clean baseline is still running. Keep batch=32 for this in-progress run; before a future formal run, a throughput-only batch probe may be used to better utilize the RTX 4090 24GB server without changing the official-val protocol.
```
