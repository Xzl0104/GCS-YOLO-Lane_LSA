...
GPU utilization sample at 2026-06-14 11:51 CST: 22%, 0%, 0%, 0%, 0%; treat as point-in-time utilization, not a reason to interrupt the healthy run
status at 2026-06-14 11:51 CST: training alive; results.csv has ordinary-val row 79, and official_best_summary has official-val candidates through epoch 79
ordinary val latest row: epoch=79, val/f1=0.952490, precision=0.948855, recall=0.956154, val/decode/count_head_k=3.65565, val/decode/final_pred_lanes=3.60881
...
official_top_k retained epochs and ACC: 73=0.955238, 76=0.955010, 61=0.954807, 72=0.954547, 79=0.954285
latest official-val candidate: epoch=79, official_acc=0.954285, FP=0.055556, FN=0.038797
diagnostic top-k notes: epoch73 remains K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.001482, countboundary 0.954137 by +0.001101, and old FT6 0.954782 by +0.000456. It remains below legacy 0.959224 by -0.003986 and below the 0.97 objective. Epoch76 came close to best and epoch79 entered Top-K, so this run has not clearly plateaued. Candidate supply and GT5 NMS remain non-dominant; the stable bottleneck is fifth-lane valid-point/output survival versus FP/GT4/GT5 count tradeoff, with gt5_valid_points_fail_rate still above the 0.1 diagnostic target and rate_4_to_5=0.090909.
errors: process is alive; results.csv has no numeric NaN/Inf values; 79 run JSON files have no numeric NaN/Inf values; official-val sweeps exist through epoch79. A text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. args.yaml records split=val and gcs_official_best_split=val.
decision: continue monitoring; K56 has beaten several active official-val references and remains active in Top-K, but remains below legacy 0.959224 and still fails GT5 diagnostic targets. Do not promote, do not use test, and do not launch a replacement experiment while the healthy baseline is still improving. Keep batch=32 for this in-progress run; before a future formal run, a throughput-only batch probe may be used to better utilize the RTX 4090 24GB server without changing the official-val protocol.
```
