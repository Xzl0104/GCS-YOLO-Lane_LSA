...
GPU utilization sample at 2026-06-14 epoch132 audit: 22%; `ps`, `results.csv`, and checkpoint updates confirm the healthy batch=32 run is still alive, so do not alter batch mid-run
status at 2026-06-14 epoch132 audit: training alive with ordinary-val row 132/180; root official_best_summary has official-val candidates through epoch 131
ordinary val latest row: epoch=132, val/f1=0.957790, precision=0.955590, recall=0.960000, fp=58, fn=52, val/decode/k5_to_output4_rate=0.111111
ordinary val best row so far by val/f1: epoch=97, val/f1=0.959724, precision=0.957154, recall=0.962308
official_best so far: epoch 127, official_acc=0.958484, FP=0.047383, FN=0.029614
official_best count/GT5 diagnostics: count_acc_3/4/5=0.928251/0.893939/0.810811, gt5_output5_rate=0.810811, gt5_count_head_under_rate=0.067568, gt5_valid_points_fail_rate=0.121622, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.013514, decode/k5_to_output4_rate=0.133333, rescue_precision=0.753846, rate_4_to_5=0.075758, rate_5_to_4=0.189189, matched/unmatched_quality_mean=0.903372/0.813948
official_top_k retained epochs and ACC: 127=0.958484, 128=0.958443, 129=0.958332, 131=0.958096, 126=0.958017
latest official-val candidate: epoch=131, official_acc=0.958096, FP=0.047337, FN=0.031910, count_acc_3/4/5=0.928251/0.878788/0.810811, gt5_output5_rate=0.810811, gt5_count_head_under_rate=0.081081, gt5_valid_points_fail_rate=0.121622, gt5_candidate_pool_shortfall_rate=0.000000, gt5_top5_suppressed_by_nms_rate=0.013514, decode/k5_to_output4_rate=0.135135, rescue_precision=0.796875, matched/unmatched_quality_mean=0.905863/0.816283
diagnostic top-k notes: epoch127 refreshes K56 official-best and exceeds the current-code K32 audit 0.953756 by +0.004728, countboundary 0.954137 by +0.004347, old FT6 0.954782 by +0.003702, prior K56 epoch83 best 0.956905 by +0.001579, and epoch115 by +0.000524. It remains below legacy 0.959224 by -0.000740 and below the 0.97 objective. Epoch128 is nearly tied with epoch127 while improving GT5 retention (`gt5_output5_rate=0.837838`, `gt5_valid_points_fail_rate=0.108108`), but neither exceeds legacy yet. Candidate supply remains healthy; GT5 NMS is low. The next K56 candidate, if needed after clearer plateau or completion, should preserve epoch127 FP/FN gains while approaching epoch128 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.
errors: process is alive; results.csv has no numeric NaN/Inf values across 132 rows; 133 run JSON files have no parse error and no numeric NaN/Inf values; official-val sweeps exist through epoch131. A text-artifact scan of 267 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. args.yaml records split=val, gcs_official_best_split=val, imgsz=[544, 960], gcs_imgsz=[544, 960], and K56 data/model. Note that the active summary is at run root `official_best_summary.json`; `weights/official_best_summary.json` is absent while `weights/official_best.pt` exists.
