...
Top-K EMA-state average sidecar: equal averaged the retained EMA state dicts for epochs 152/165/166/168/170 under weights/swa_ema_topk/top5_equal_epochs152_165_166_168_170.pt, with manifest weights/swa_ema_topk/manifest.json. Independent official-val sweep at analysis_swa_ema_top5_val_sweep used the same 64-combo validation protocol and selected the same decode row (conf=0.005, point_valid_thr=0.35, nms_dist_px=18.0, max_det=5, min_points=6, rank_min_points=none), but reached only official_acc=0.959068, FP=0.044858, FN=0.028237. It improved count_acc_3/4/5 to 0.932735/0.893939/0.878378, gt5_output5_rate to 0.878378, rate_4_to_5 to 0.060606, rate_5_to_4 to 0.121622, and rescue_precision to 0.797101, but missed the official_acc>0.9600 target and underperformed epoch152 by -0.000247. GT5 diagnosis at analysis_swa_ema_top5_gt5_diag_val kept=65/74, count_head_under_predict=5, quality_too_low=4, candidate_pool_shortfall=0, GT5 NMS suppression=0, rank5_score_low=0, valid_points_fail=0.
...
decision: K56 baseline is a stronger official-val reference, but not a final-test candidate yet. The Top-K EMA-state average is diagnostic-only: it smooths count/quality tradeoffs but does not beat epoch152 or reach 0.9600, so do not promote it and do not expand into random averaging-recipe search. Do not use test. Do not rerun the two rejected K56 Count/Quality gates below.
```
...
The K56 official-val evidence is a completed baseline result, not a mainline promotion and not a reason to use test or tune postprocess settings. The Top-K EMA-state average is also not promotable despite better GT5 retention and lower `rate_4_to_5`, because official_acc fell from `0.959315` to `0.959068`. The next useful K56 step should be a more targeted training-side geometry or Count/Quality change that preserves the epoch152 FP/FN balance before attempting another official-val gate.
