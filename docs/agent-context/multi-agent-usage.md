...
This branch uses a single project-wide delegation rule for Codex Agents. These tools are synchronized from the main GCS-YOLO-Lane project, but all agents must follow this branch's `5-25-3.zip` + Q12/K56 fixed-y contracts and must not treat later mainline Count/Quality/Boundary mechanisms as active behavior unless a future task explicitly changes the algorithm scope.
...
Every project Agent must preserve:
