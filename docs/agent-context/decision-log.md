...
## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 32 monitor
...
A read-only remote check at `2026-06-14 09:03 CST` found the process alive, using about `17.9 GiB` on the RTX 4090 24GB server. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. Log keyword scanning found no OOM, NaN, traceback, runtime error, CUDA error, assertion, shape mismatch, size mismatch, or invalid shape.

`results.csv` has an ordinary-validation row labeled epoch `32`:

```text
val/f1: 0.946636
val/decode/count_head_k: 3.60055
val/decode/final_pred_lanes: 3.54270
val/decode/k5_to_output4_rate: 0.276316
```

The best ordinary-validation row so far by `val/f1` is epoch `31`:

```text
val/f1: 0.950366
val/decode/count_head_k: 3.64738
val/decode/final_pred_lanes: 3.57851
val/decode/k5_to_output4_rate: 0.297619
```

`official_best_summary.json` has official-val candidates through epoch `31`. The current official-best row is:
...
This is a material improvement over the epoch 23 best (`official_acc=0.944138`) and the epoch 28 best (`official_acc=0.948286`), but it is still below the legacy `0.959224` reference. The current evidence still points to a GT5 valid-point/output-vs-FP tradeoff rather than candidate-pool shortfall.
...
- Launch a parallel K56 auxiliary experiment immediately.
- Use test to look for a better checkpoint or postprocess setting.
- Continue the healthy baseline while official-val is still improving.

Tradeoff:

Continuing uses server time, but the baseline is healthy and still improving. Launching a replacement now would compete for the same 24GB GPU and could overfit the next hypothesis before the clean K56 baseline matures. If this run plateaus below legacy, the smallest next controlled candidate should target weak training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint.
...
## 2026-06-14: Continue Q12-K56 b32 baseline after ordinary epoch 49 monitor
...
A read-only remote check at `2026-06-14 10:05 CST` found the process alive, using about `17.5 GiB` on the RTX 4090 24GB server. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`.

`results.csv` has an ordinary-validation row labeled epoch `49`:

```text
val/f1: 0.951210
val/decode/count_head_k: 3.62810
val/decode/final_pred_lanes: 3.58953
val/decode/k5_to_output4_rate: 0.186667
```

The best ordinary-validation row so far by `val/f1` remains epoch `39`:
...
`official_best_summary.json` has official-val candidates through epoch `48`. The current official-best row is now:

```text
best_epoch: 43
official_acc: 0.951958
official_fp: 0.067264
official_fn: 0.047521
count_acc_3/4/5: 0.901345 / 0.878788 / 0.824324
gt5_output5_rate: 0.824324
gt5_count_head_under_rate: 0.013514
gt5_valid_points_fail_rate: 0.162162
gt5_candidate_pool_shortfall_rate: 0.000000
decode/k5_to_output4_rate: 0.244186
rescue_precision: 0.800000
rate_4_to_5: 0.060606
```
...
Epoch 43 is a small official-val improvement over epoch 31 (`0.951958` versus `0.951526`) and improves GT5 output, but it remains below the legacy `0.959224` target and carries higher FP. Epoch 48 is diagnostically useful because it has lower FN (`0.042470`) and stronger GT5 output (`0.837838`), but it is not promotable because its official Accuracy is only `0.950146` and GT4-to-5 pressure rises (`rate_4_to_5=0.090909`).

Alternatives considered:

- Stop the run because official-val remains below the legacy `0.959224` target.
- Launch a parallel K56 auxiliary experiment immediately.
- Promote epoch 48 because GT5 output and FN are better.
- Use test to look for a better checkpoint or postprocess setting.
- Continue the healthy baseline while official-val is still moving.

Tradeoff:

Continuing uses server time, but the baseline is healthy and has a new official-best at epoch 43. Starting an auxiliary experiment now would compete for the same 24GB GPU and risks overreacting before the first clean K56 baseline matures. If this run plateaus below legacy, the smallest next controlled candidate should target weak training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, `official_best_summary.json`, official-val sweep summaries through epoch 48, retained Top-K metadata, and that `results.csv` contains no NaN/Inf values. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 52 official-best
...
A read-only remote check at `2026-06-14 10:18 CST` found the process alive, using about `17.5 GiB` on the RTX 4090 24GB server. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`.

`results.csv` has an ordinary-validation row labeled epoch `53`:

```text
val/f1: 0.945636
```

The best ordinary-validation row so far by `val/f1` is epoch `51`:

```text
val/f1: 0.952859
val/decode/count_head_k: 3.61708
val/decode/final_pred_lanes: 3.54821
val/decode/k5_to_output4_rate: 0.333333
```

`official_best_summary.json` has official-val candidates through epoch `52`. The current official-best row is now:
...
Epoch 52 is a material official-val improvement over epoch 43 (`0.953566` versus `0.951958`) and closes much of the gap to the current-code K32 audit baseline (`0.953756`). It is still not promotable because it remains below the current-code K32 audit, countboundary (`0.954137`), old FT6 (`0.954782`), and legacy (`0.959224`) references. The remaining evidence still points to fifth-lane valid-point/output survival versus FP/GT4-to-5 pressure: `gt5_valid_points_fail_rate=0.135135` is above the diagnostic `0.1` target, `gt5_top5_suppressed_by_nms_rate=0.013514`, and `rate_4_to_5=0.075758`.

Alternatives considered:

- Stop the run because official-val remains below the legacy `0.959224` target.
- Launch a parallel K56 auxiliary experiment immediately.
- Promote epoch 52 because it is the new official-best.
- Use test to look for a better checkpoint or postprocess setting.
- Continue the healthy baseline while official-val is still moving.

Tradeoff:

Continuing uses server time, but the baseline is healthy, only around epoch 53/180, and just refreshed official-best at epoch 52. Starting an auxiliary experiment now would compete for the same 24GB GPU before the first clean K56 baseline matures. If this run plateaus below legacy, the smallest next controlled candidate should target weak training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, `official_best_summary.json`, official-val sweep summaries through epoch 52, retained Top-K metadata, and that `results.csv` contains no NaN/Inf values. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after official-val epoch 54 monitor
...
A read-only remote check at `2026-06-14 10:25 CST` found the process alive, using about `17.5 GiB` on the RTX 4090 24GB server. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`.

`results.csv` has an ordinary-validation row labeled epoch `55`:

```text
val/f1: 0.943657
val/decode/count_head_k: 3.65840
val/decode/final_pred_lanes: 3.60606
val/decode/k5_to_output4_rate: 0.211111
```

The best ordinary-validation row so far by `val/f1` remains epoch `51`:
...
`official_best_summary.json` has official-val candidates through epoch `54`. The current official-best row remains:
...
Epochs 53 and 54 entered Top-K (`official_acc=0.952030` and `0.951580`) but did not beat epoch 52. Epoch 52 remains close to the current-code K32 audit baseline (`0.953756`) but is still below it, the countboundary baseline (`0.954137`), the old FT6 reference (`0.954782`), and the legacy `0.959224` reference. The remaining evidence still points to fifth-lane valid-point/output survival versus FP/GT4-to-5 pressure: `gt5_valid_points_fail_rate=0.135135` is above the diagnostic `0.1` target, `gt5_top5_suppressed_by_nms_rate=0.013514`, and `rate_4_to_5=0.075758`.

Alternatives considered:

- Stop the run because official-val remains below the legacy `0.959224` target.
- Launch a parallel K56 auxiliary experiment immediately.
- Promote epoch 52 because it is close to the current-code K32 audit baseline.
- Use test to look for a better checkpoint or postprocess setting.
- Continue the healthy baseline while official-val is still moving.

Tradeoff:

Continuing uses server time, but the baseline is healthy, only around epoch 55/180, and official-val has not plateaued. Starting an auxiliary experiment now would compete for the same 24GB GPU before the first clean K56 baseline matures. If this run plateaus below legacy, the smallest next controlled candidate should target weak training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, `official_best_summary.json`, official-val sweep summaries through epoch 54, retained Top-K metadata, and that `results.csv` and official-val JSON summaries contain no numeric NaN/Inf values. No test evidence was used.
...
## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 61 official-best
...
A read-only main-agent remote check and a read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 10:52 CST` found the process alive. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU samples showed about `17.5` to `18.2 GiB` in use on the RTX 4090 24GB server.

`results.csv` has an ordinary-validation row labeled epoch `62`:

```text
val/f1: 0.951426
val/precision: 0.953632
val/recall: 0.949231
val/decode/count_head_k: 3.61157
val/decode/final_pred_lanes: 3.56474
val/decode/k5_to_output4_rate: 0.223684
val/decode/candidate_pool_shortfall_rate: 0.000000
val/decode/top5_suppressed_by_nms_rate: 0.071625
```

The best ordinary-validation row so far by `val/f1` remains epoch `58`:
...
`official_best_summary.json` has official-val candidates through epoch `62`. The current official-best row is now:

```text
best_epoch: 61
official_acc: 0.954807
official_fp: 0.053489
official_fn: 0.041093
count_acc_3/4/5: 0.923767 / 0.878788 / 0.729730
gt5_output5_rate: 0.729730
gt5_count_head_under_rate: 0.081081
gt5_valid_points_fail_rate: 0.189189
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.287500
rescue_precision: 0.807018
rate_4_to_5: 0.045455
...
This is the first K56 baseline checkpoint to exceed the current-code K32 audit (`0.953756`), countboundary (`0.954137`), and old FT6 (`0.954782`) references under official-val. It is still below legacy `0.959224` and the 0.97 objective. It is also still diagnostically weak on fifth-lane survival: `gt5_output5_rate=0.729730`, `gt5_valid_points_fail_rate=0.189189`, and `rate_5_to_4=0.270270`.

Epoch `59` is important diagnostically:

```text
official_acc: 0.952904
gt5_output5_rate: 0.837838
gt5_valid_points_fail_rate: 0.094595
rate_4_to_5: 0.030303
rate_5_to_4: 0.162162
```

It satisfies the diagnostic valid-points target but loses official ACC, so the current bottleneck is still fifth-lane valid-point/output survival versus FP/GT4/GT5 count tradeoff, not candidate supply or GT5 NMS.

Alternatives considered:

- Stop the run because it has now beaten old FT6.
- Promote epoch 61 and run final test immediately.
- Launch a parallel K56 auxiliary experiment immediately from epoch 61.
- Use test to see whether epoch 61 generalizes better than official-val suggests.
- Continue the healthy baseline while it is still moving.

Tradeoff:

Continuing uses server time, but the baseline is healthy, only around epoch 62/180, and official-val has just found a new best. Starting an auxiliary experiment now would compete for the same 24GB GPU before the first clean K56 baseline matures. If this run plateaus below legacy, the smallest next controlled candidate should target training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, `official_best_summary.json`, official-val sweep summaries through epoch 62, retained Top-K metadata, and that `results.csv` plus 63 official-val JSON summaries contain no numeric NaN/Inf values. A text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after ordinary epoch 66 monitor
...
A read-only main-agent remote check at `2026-06-14 11:03 CST` and a read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 11:05 CST` found the process alive. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU samples showed about `17.5` to `18.2 GiB` in use on the RTX 4090 24GB server.

`results.csv` has an ordinary-validation row labeled epoch `66`:

```text
val/f1: 0.954178
val/precision: 0.955281
val/recall: 0.953077
val/decode/count_head_k: 3.62259
val/decode/final_pred_lanes: 3.57300
val/decode/k5_to_output4_rate: 0.240000
val/decode/candidate_pool_shortfall_rate: 0.000000
val/decode/top5_suppressed_by_nms_rate: 0.057851
```

The best ordinary-validation row so far by `val/f1` is now epoch `65`:

```text
val/f1: 0.957390
val/decode/count_head_k: 3.62810
val/decode/final_pred_lanes: 3.59504
val/decode/k5_to_output4_rate: 0.162162
```

`official_best_summary.json` has official-val candidates through epoch `65`. The current official-best row remains epoch `61`:
...
The retained official Top-K epochs remain:

```text
61, 52, 58, 62, 59
```

Epoch `65` refreshed ordinary `val/f1`, but its official-val candidate was not promotable:

```text
official_acc: 0.952533
official_fp: 0.054637
official_fn: 0.040404
gt5_output5_rate: 0.743243
gt5_valid_points_fail_rate: 0.189189
```

This confirms again that ordinary validation fitness is not a safe promotion signal for TuSimple official Accuracy. The current official-val best still beats the current-code K32 audit, countboundary, and old FT6 references, but remains below legacy `0.959224` and the 0.97 objective. The persistent bottleneck is fifth-lane survival versus FP/GT4/GT5 tradeoff, not candidate supply or GT5 NMS.

Alternatives considered:

- Promote epoch 65 because ordinary `val/f1` is best so far.
- Stop at epoch 61 because it beat old FT6.
- Launch a parallel K56 auxiliary experiment immediately.
- Use test to choose between epoch 61 and later ordinary-best checkpoints.
- Continue the healthy baseline while official-val is still active.

Tradeoff:

Continuing uses server time, but the run is healthy and only around epoch 66/180. Launching a competing experiment now would consume the same 24GB GPU before the first clean K56 baseline matures. If this run plateaus below legacy, the next controlled candidate should target training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, `official_best_summary.json`, official-val sweep summaries through epoch 65, retained Top-K metadata, and that `results.csv` plus 66 official-val JSON summaries contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after official-val epoch 70 monitor
...
A read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 11:20 CST` found the process alive. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was `17928 / 24564 MiB` on the RTX 4090 24GB server. Five point-in-time utilization samples were `19%, 0%, 0%, 0%, 0%`; keep the healthy run alive and use a throughput-only batch probe before a future formal run if more server GPU capacity should be used.

`results.csv` has an ordinary-validation row labeled epoch `71`:

```text
val/f1: 0.951248
val/precision: 0.949425
val/recall: 0.953077
val/decode/count_head_k: 3.66391
val/decode/final_pred_lanes: 3.59504
val/decode/k5_to_output4_rate: 0.290698
val/decode/candidate_pool_shortfall_rate: 0.000000
val/decode/top5_suppressed_by_nms_rate: 0.033058
```

The best ordinary-validation row so far by `val/f1` is now epoch `67`:

```text
val/f1: 0.958157
```

`official_best_summary.json` has official-val candidates through epoch `70`. The current official-best row remains epoch `61`:
...
The latest official-val candidate is epoch `70`:

```text
official_acc: 0.952543
official_fp: 0.049770
official_fn: 0.041781
count_acc_3/4/5: 0.946188 / 0.863636 / 0.729730
gt5_output5_rate: 0.729730
gt5_valid_points_fail_rate: 0.216216
```

Epochs `69`, `68`, and `67` entered official Top-K but did not beat epoch `61`. Epoch `67` is the best ordinary `val/f1` row so far, which again shows that ordinary validation fitness is not a safe promotion signal for TuSimple official Accuracy. The current official-val best still beats the current-code K32 audit, countboundary, and old FT6 references, but remains below legacy `0.959224` and the 0.97 objective. The persistent bottleneck is fifth-lane survival versus FP/GT4/GT5 tradeoff, not candidate supply or GT5 NMS.

Alternatives considered:

- Promote epoch 67 because ordinary `val/f1` is best so far.
- Stop at epoch 61 because it beat old FT6.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Launch a parallel K56 auxiliary experiment immediately.
- Use test to choose between epoch 61 and later Top-K checkpoints.
- Continue the healthy baseline while official-val is still active.

Tradeoff:

Continuing uses server time, but the run is healthy and only around epoch 71/180. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing experiment would consume the same 24GB GPU before the first clean K56 baseline matures. If this run plateaus below legacy, the next controlled candidate should target training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint, with a separate throughput-only batch probe before the next formal run if resource use should be increased.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 70, retained Top-K metadata, and that `results.csv` plus 71 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 73 official-best
...
A read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 11:37 CST` found the process alive. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17928 / 24564 MiB` on the RTX 4090 24GB server. Utilization point samples ranged from `0-22%` to `57%`; treat that as point-in-time utilization, not a reason to interrupt the healthy in-progress run.

`results.csv` has an ordinary-validation row labeled epoch `75`:

```text
val/f1: 0.955684
val/precision: 0.957529
val/recall: 0.953846
val/decode/count_head_k: 3.61984
val/decode/final_pred_lanes: 3.56749
val/decode/k5_to_output4_rate: 0.246753
val/decode/candidate_pool_shortfall_rate: 0.002755
val/decode/top5_suppressed_by_nms_rate: 0.063361
```

The best ordinary-validation row so far by `val/f1` is now epoch `73`:

```text
val/f1: 0.959693
val/precision: 0.957854
val/recall: 0.961538
```

`official_best_summary.json` has official-val candidates through epoch `74`. The current official-best row is now epoch `73`:

```text
best_epoch: 73
official_acc: 0.955238
official_fp: 0.046465
official_fn: 0.031910
count_acc_3/4/5: 0.937220 / 0.848485 / 0.810811
gt5_output5_rate: 0.810811
gt5_count_head_under_rate: 0.040541
gt5_valid_points_fail_rate: 0.148649
gt5_candidate_pool_shortfall_rate: 0.013514
gt5_top5_suppressed_by_nms_rate: 0.013514
decode/k5_to_output4_rate: 0.172840
rescue_precision: 0.761194
rate_4_to_5: 0.090909
```
...
The latest official-val candidate is epoch `74`:

```text
official_acc: 0.953752
official_fp: 0.051653
official_fn: 0.041552
```

Epoch `73` refreshes K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.001482`, countboundary (`0.954137`) by `+0.001101`, and old FT6 (`0.954782`) by `+0.000456`. It remains below legacy `0.959224` by `-0.003986` and below the 0.97 objective. It improves GT5 output over epoch `61` (`0.810811` vs `0.729730`) and lowers FN (`0.031910`), but `gt5_valid_points_fail_rate=0.148649` remains above the `0.1` diagnostic target and `rate_4_to_5=0.090909` shows GT4-to-5 FP pressure.

Alternatives considered:

- Promote epoch 73 because it is the new official-best.
- Stop at epoch 73 and run final test.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Launch a parallel K56 auxiliary experiment immediately.
- Use test to choose between epoch 73 and later Top-K checkpoints.
- Continue the healthy baseline while official-val is still active.

Tradeoff:

Continuing uses server time, but the run is healthy and only around epoch 75/180, and official-val just refreshed the best checkpoint. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing experiment would consume the same 24GB GPU before the first clean K56 baseline matures. If this run plateaus below legacy, the next controlled candidate should target training-side GT5 edge visible-segment point-valid support and Count/Quality calibration from an official-val-selected K56 checkpoint, with a separate throughput-only batch probe before the next formal run if resource use should be increased.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 74, retained Top-K metadata, and that `results.csv` plus 75 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val` and `gcs_official_best_split=val`. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after official-val epoch 79 monitor
...
A read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 11:51 CST` found the process alive. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17928 / 24564 MiB` on the RTX 4090 24GB server. Five utilization samples were `22%, 0%, 0%, 0%, 0%`; treat them as point-in-time utilization, not a reason to interrupt the healthy in-progress run.

`results.csv` has an ordinary-validation row labeled epoch `79`:

```text
val/f1: 0.952490
val/precision: 0.948855
val/recall: 0.956154
val/decode/count_head_k: 3.65565
val/decode/final_pred_lanes: 3.60881
```

The best ordinary-validation row so far by `val/f1` remains epoch `73`:
...
`official_best_summary.json` has official-val candidates through epoch `79`. The current official-best row remains epoch `73`:
...
decode/k5_to_output4_rate: 0.172840
rescue_precision: 0.761194
matched/unmatched quality mean: 0.873611 / 0.769236
rate_4_to_5: 0.090909
rate_5_to_4: 0.189189
```
...
The latest official-val candidate is epoch `79`:

```text
official_acc: 0.954285
official_fp: 0.055556
official_fn: 0.038797
```

Epoch `73` still exceeds the current-code K32 audit (`0.953756`) by `+0.001482`, countboundary (`0.954137`) by `+0.001101`, and old FT6 (`0.954782`) by `+0.000456`. It remains below legacy `0.959224` by `-0.003986` and below the 0.97 objective. Epoch `76` is close to best (`0.955010`) and epoch `79` entered Top-K (`0.954285`), so the run is not clearly plateaued. The persistent bottleneck remains GT5 fifth-lane valid-point/output survival versus FP/GT4-to-5 tradeoff, not raw candidate supply, NMS, or test/threshold selection.

Alternatives considered:

- Promote epoch 73 because it remains the official-best.
- Stop at epoch 79 because later epochs have not beaten epoch 73.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Launch a parallel K56 auxiliary experiment immediately.
...
Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 79, retained Top-K metadata, and that `results.csv` plus 79 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val` and `gcs_official_best_split=val`. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 82 official-best
...
A read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 12:05-12:06 CST` found the process alive. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17928 / 24564 MiB` on the RTX 4090 24GB server. Five utilization samples were `10%, 19%, 3%, 0%, 0%`; treat them as point-in-time utilization, not a reason to interrupt the healthy in-progress run.

`results.csv` has an ordinary-validation row labeled epoch `83`:

```text
val/f1: 0.958093
val/precision: 0.957725
val/recall: 0.958462
val/decode/count_head_k: 3.60881
val/decode/final_pred_lanes: 3.58402
```
...
`official_best_summary.json` has official-val candidates through epoch `82`. The current official-best row is now epoch `82`:

```text
best_epoch: 82
official_acc: 0.955292
official_fp: 0.052525
official_fn: 0.036042
count_acc_3/4/5: 0.919283 / 0.878788 / 0.891892
gt5_output5_rate: 0.891892
gt5_count_head_under_rate: 0.040541
gt5_valid_points_fail_rate: 0.067568
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.135802
rescue_precision: 0.814286
rate_3_to_4: 0.080717
rate_4_to_5: 0.060606
rate_5_to_4: 0.108108
```
...
Epoch `82` refreshes K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.001536`, countboundary (`0.954137`) by `+0.001155`, and old FT6 (`0.954782`) by `+0.000510`. It remains below legacy `0.959224` by `-0.003932` and below the 0.97 objective. The gain over epoch `73` is real but tiny (`+0.000054` official ACC). Its main value is diagnostic: GT5 survival is much better than epoch `73` (`gt5_output5_rate=0.891892`, `gt5_valid_points_fail_rate=0.067568`, GT5 candidate shortfall/NMS `0.0`), while the remaining gap is FP and GT3/GT4 over-count pressure.

Alternatives considered:

- Promote epoch 82 because it is the new official-best.
- Stop at epoch 82 and run final test.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Launch a parallel K56 auxiliary experiment immediately.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val is still active.
...
Continuing uses server time, but the run is healthy and only around epoch 83/180, and official-val just refreshed the best checkpoint. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing experiment would consume the same 24GB GPU before the first clean K56 baseline matures. If this run later plateaus below legacy, the next controlled candidate should preserve epoch-82 GT5 survival while reducing FP and GT3/GT4 over-count pressure via training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 82, retained Top-K metadata, and that `results.csv` plus 83 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val` and `gcs_official_best_split=val`. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 83 official-best
...
A read-only `gcs_experiment_analyst` subagent audit at `2026-06-14 12:19 CST` found the process alive around epoch `88/180`. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17928 / 24564 MiB` on the RTX 4090 24GB server. Five utilization samples were `0%, 3%, 0%, 22%, 0%`; treat them as point-in-time utilization, not a reason to interrupt the healthy in-progress run.

`results.csv` has an ordinary-validation row labeled epoch `88`:

```text
val/f1: 0.955888
val/precision: 0.953328
val/recall: 0.958462
val/decode/count_head_k: 3.64463
val/decode/final_pred_lanes: 3.60055
val/decode/k5_to_output4_rate: 0.207792
```
...
`official_best_summary.json` has official-val candidates through epoch `87`. The current official-best row is now epoch `83`:

```text
best_epoch: 83
official_acc: 0.956905
official_fp: 0.042149
official_fn: 0.031680
count_acc_3/4/5: 0.941704 / 0.909091 / 0.810811
gt5_output5_rate: 0.810811
gt5_count_head_under_rate: 0.094595
gt5_valid_points_fail_rate: 0.108108
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.186667
rescue_precision: 0.852459
rate_3_to_4: 0.058296
rate_4_to_5: 0.030303
rate_5_to_4: 0.189189
matched/unmatched quality mean: 0.874704 / 0.782380
```
...
Epoch `83` refreshes K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.003149`, countboundary (`0.954137`) by `+0.002768`, and old FT6 (`0.954782`) by `+0.002123`. It remains below legacy `0.959224` by `-0.002319` and below the 0.97 objective. The gain over epoch `82` is `+0.001613` official ACC and comes with lower FP/FN, but GT5 output drops from epoch `82` (`0.891892`) to `0.810811`, and `gt5_valid_points_fail_rate=0.108108` remains slightly above the diagnostic `0.1` line. Candidate supply and GT5 NMS remain non-dominant.

Alternatives considered:

- Promote epoch 83 because it is the new official-best.
- Stop at epoch 83 and run final test.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Launch a parallel K56 auxiliary experiment immediately.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val is still active.
...
Continuing uses server time, but the run is healthy, only around epoch `88/180`, and official-val just refreshed the best checkpoint. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing experiment would consume the same 24GB GPU before the first clean K56 baseline matures. If this run later plateaus below legacy, the next controlled candidate should preserve epoch-83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 87, retained Top-K metadata, and that `results.csv` plus 88 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val` and `gcs_official_best_split=val`. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 94 Top-K monitor
...
Do not stop it, do not launch a replacement Count/Quality calibration yet, do not promote K56 yet, and do not use test for checkpoint, threshold, loss, or postprocess selection.

Why:

A read-only `gcs_experiment_analyst` subagent audit and main-agent remote check at `2026-06-14 12:45 CST` found the process alive around epoch `95/180`. The command is still the intended formal K56 baseline with `--batch 32 --workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `18640 / 24564 MiB` during active training on the RTX 4090 24GB server, and six utilization samples were `100%, 18%, 97%, 18%, 68%, 100%`.

`results.csv` has an ordinary-validation row labeled epoch `95`:

```text
val/f1: 0.955086
val/precision: 0.953257
val/recall: 0.956923
val/decode/count_head_k: 3.63636
val/decode/final_pred_lanes: 3.59504
val/decode/k5_to_output4_rate: 0.194805
```
...
`official_best_summary.json` has official-val candidates through epoch `94`. The current official-best row remains epoch `83`:
...
The latest official-val candidate is epoch `94`:

```text
official_acc: 0.955354
official_fp: 0.050918
official_fn: 0.036272
count_acc_3/4/5: 0.923767 / 0.893939 / 0.810811
gt5_output5_rate: 0.810811
gt5_count_head_under_rate: 0.094595
gt5_valid_points_fail_rate: 0.094595
gt5_candidate_pool_shortfall_rate: 0.013514
gt5_top5_suppressed_by_nms_rate: 0.000000
rescue_precision: 0.828125
matched/unmatched quality mean: 0.882169 / 0.755599
rate_3_to_4: 0.076233
rate_4_to_5: 0.060606
rate_5_to_4: 0.189189
```
...
Epoch `94` entering Top-K confirms official-val is still active, but it does not beat epoch `83`. Epoch `83` still exceeds the current-code K32 audit (`0.953756`) by `+0.003149`, countboundary (`0.954137`) by `+0.002768`, and old FT6 (`0.954782`) by `+0.002123`. It remains below legacy `0.959224` by `-0.002319` and below the 0.97 objective. Epoch `94` improves `gt5_valid_points_fail_rate` versus epoch `83` (`0.094595` vs `0.108108`) but pays higher FP/FN and candidate shortfall. Candidate supply and GT5 NMS remain non-dominant.

Alternatives considered:

- Promote epoch 83 because it remains the official-best.
- Stop at epoch 95 because no later epoch has beaten epoch 83.
- Start K56 Count/Quality calibration immediately from epoch 83.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
...
Continuing uses server time, but the run is healthy, only around epoch `95/180`, and official-val still has Top-K movement. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch-83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory and utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 94, retained Top-K metadata, and that `results.csv` plus 95 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan inside the run found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, and `imgsz=[544, 960]`. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 101 Top-K monitor
...
A read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `18640 / 24564 MiB` on the RTX 4090 24GB server, with utilization samples `35%, 75%, 15%, 94%, 100%, 18%, 62%, 82%, 17%, 39%`. This rules out the immediate concern that `batch=32` is obviously wasting server VRAM or continuously idling.

`results.csv` has an ordinary-validation row labeled epoch `102`:

```text
val/f1: 0.956120
val/precision: 0.956857
val/recall: 0.955385
val/FP: 56
val/FN: 58
val/decode/count_head_k: 3.60881
val/decode/final_pred_lanes: 3.57576
val/decode/k5_to_output4_rate: 0.173913
```

The best ordinary-validation row so far by `val/f1` is now epoch `97`:
...
`official_best_summary.json` at the run root has official-val candidates through epoch `101`. The current official-best row remains epoch `83`:
...
The latest official-val candidate is epoch `101`:

```text
official_acc: 0.956168
official_fp: 0.048990
official_fn: 0.036501
count_acc_3/4/5: 0.928251 / 0.893939 / 0.783784
gt5_output5_rate: 0.783784
gt5_count_head_under_rate: 0.108108
gt5_valid_points_fail_rate: 0.108108
gt5_candidate_pool_shortfall_rate: 0.013514
gt5_top5_suppressed_by_nms_rate: 0.000000
...
Epoch `83` still exceeds the current-code K32 audit (`0.953756`) by `+0.003149`, countboundary (`0.954137`) by `+0.002768`, and old FT6 (`0.954782`) by `+0.002123`. It remains below legacy `0.959224` by `-0.002319` and below the 0.97 objective. Epoch `97` becoming the ordinary-val best and the second official Top-K row shows the run is still producing meaningful candidates after epoch `83`; epoch `101` entering Top-K reinforces that this is not a complete plateau yet.

Alternatives considered:

- Promote epoch 83 because it remains the official-best.
- Stop at epoch 102 because no later epoch has beaten epoch 83.
- Start K56 Count/Quality calibration immediately from epoch 83.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `102/180`, and official-val still has Top-K movement. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch-83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory and utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 101, retained Top-K metadata, and that `results.csv` plus 102 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan of 205 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, and `gcs_imgsz=[544, 960]`. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 107 Top-K monitor
...
A read-only `gcs_experiment_analyst` subagent audit plus main-agent read-only remote check on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17.9-18.6 / 24.6 GiB` on the RTX 4090 24GB server. Utilization samples ranged from `0%` to `100%`; combined with memory use and continuing log updates, this is not a reason to interrupt or change `batch` mid-run.

`results.csv` has an ordinary-validation row labeled epoch `108`:

```text
val/f1: 0.956890
val/precision: 0.957627
val/recall: 0.956154
val/decode/count_head_k: 3.61157
val/decode/final_pred_lanes: 3.57576
val/decode/k5_to_output4_rate: 0.180556
```
...
`official_best_summary.json` at the run root has official-val candidates through epoch `107`. The current official-best row remains epoch `83`:
...
The latest official-val candidate is epoch `107`:

```text
official_acc: 0.956640
official_fp: 0.042424
official_fn: 0.031221
count_acc_3/4/5: 0.937220 / 0.863636 / 0.743243
gt5_output5_rate: 0.743243
gt5_count_head_under_rate: 0.108108
gt5_valid_points_fail_rate: 0.162162
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
rescue_precision: 0.793103
```
...
Epoch `107` entered Top-K second and is only `0.000265` below epoch `83`, but it does not beat the official-best and it worsens GT5 output/valid-points survival. Epoch `83` still exceeds the current-code K32 audit (`0.953756`) by `+0.003149`, countboundary (`0.954137`) by `+0.002768`, and old FT6 (`0.954782`) by `+0.002123`. It remains below legacy `0.959224` by `-0.002319` and below the 0.97 objective.

Alternatives considered:

- Promote epoch 83 because it remains the official-best.
- Stop at epoch 108 because no later epoch has beaten epoch 83.
- Start K56 Count/Quality calibration immediately from epoch 83.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `108/180`, and official-val still has Top-K movement. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch-83 low FP/FN while recovering higher GT5 output through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 107, retained Top-K metadata, and that `results.csv` plus 107 run JSON files contain no numeric NaN/Inf values in the subagent audit. A scoped text-artifact scan of 215 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 109 official-best
...
A read-only `gcs_experiment_analyst` subagent audit on `2026-06-14T13:45:30+08:00` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. A follow-up read-only main-agent check saw ordinary validation advance to epoch `113` and official-val summary advance to epoch `112`.

GPU memory was about `17928 / 24564 MiB` on the RTX 4090 24GB server. Six utilization samples were `0%, 2%, 0%, 22%, 2%, 0%`; this is a short sampling window and is not a reason to interrupt or change `batch` mid-run.

`results.csv` has an ordinary-validation row labeled epoch `113`:

```text
val/f1: 0.957291
val/precision: 0.957660
val/recall: 0.956923
val/decode/count_head_k: 3.61708
val/decode/final_pred_lanes: 3.57851
val/decode/k5_to_output4_rate: 0.191781
```
...
`official_best_summary.json` at the run root has official-val candidates through epoch `112`. The current official-best row is now epoch `109`:

```text
best_epoch: 109
official_acc: 0.957664
official_fp: 0.051056
official_fn: 0.035124
count_acc_3/4/5: 0.923767 / 0.833333 / 0.783784
gt5_output5_rate: 0.783784
gt5_count_head_under_rate: 0.094595
gt5_valid_points_fail_rate: 0.121622
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.148649
rescue_precision: 0.761905
rate_3_to_4: 0.076233
rate_4_to_5: 0.090909
rate_5_to_4: 0.216216
matched/unmatched quality mean: 0.892759 / 0.789271
```

The latest official-val candidate is epoch `112`:

```text
official_acc: 0.956981
official_fp: 0.046648
official_fn: 0.035813
count_acc_3/4/5: 0.928251 / 0.909091 / 0.716216
gt5_output5_rate: 0.716216
gt5_count_head_under_rate: 0.108108
gt5_valid_points_fail_rate: 0.175676
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
rescue_precision: 0.854545
```
...
Epoch `109` refreshes K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.003908`, countboundary (`0.954137`) by `+0.003527`, old FT6 (`0.954782`) by `+0.002882`, and prior K56 epoch83 best (`0.956905`) by `+0.000759`. It remains below legacy `0.959224` by `-0.001560` and below the 0.97 objective. The improvement comes with higher FP, weaker GT4/GT5 count retention, and a higher GT5 valid-points failure rate than the old epoch83 best.

Alternatives considered:

- Promote epoch 109 because it is the new official-best.
- Stop at epoch 113 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 109.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `113/180`, and official-val just refreshed the best checkpoint. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch-109 official ACC gain while reducing FP, GT4->5 over-count, and GT5 valid-points failure through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 112, retained Top-K metadata, and that `results.csv` plus 113 run JSON files contain no numeric NaN/Inf values. A scoped text-artifact scan of 225 run files found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.
...
## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 120 GT5-retention monitor
...
A read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17.9-18.6 / 24.6 GiB`; utilization samples were `17%, 100%, 100%, 17%, 63%`. This supports continuing the healthy `batch=32` run rather than restarting or changing the batch mid-run.

`results.csv` has an ordinary-validation row labeled epoch `121`:

```text
val/f1: 0.957790
val/precision: 0.955590
val/recall: 0.960000
val/fp: 58
val/fn: 52
val/decode/k5_to_output4_rate: 0.094595
```
...
`official_best_summary.json` at the run root has official-val candidates through epoch `120`. The current official-best row remains epoch `115`:
...
The latest official-val candidate is epoch `120`:

```text
official_acc: 0.957116
official_fp: 0.049311
official_fn: 0.035583
count_acc_3/4/5: 0.932735 / 0.863636 / 0.837838
gt5_output5_rate: 0.837838
gt5_count_head_under_rate: 0.094595
gt5_valid_points_fail_rate: 0.081081
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.109589
rescue_precision: 0.784615
matched/unmatched quality mean: 0.900745 / 0.808311
```
...
Epoch `120` is useful diagnostically because it restores GT5 retention (`gt5_output5_rate=0.837838`, `gt5_valid_points_fail_rate=0.081081`, `decode/k5_to_output4_rate=0.109589`) but it does not beat epoch `115` official ACC because FP/FN rise. Epoch `115` still exceeds the current-code K32 audit (`0.953756`) by `+0.004204`, countboundary (`0.954137`) by `+0.003823`, old FT6 (`0.954782`) by `+0.003178`, prior K56 epoch83 best (`0.956905`) by `+0.001055`, and epoch109 (`0.957664`) by `+0.000296`. It remains below legacy `0.959224` by `-0.001264` and below the 0.97 objective.

Alternatives considered:

- Promote epoch 115 because it remains the official-best.
- Stop at epoch 121 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 115.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `121/180`, and epoch120 shows the late-run GT5-retention side of the tradeoff without an official ACC win. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch-115 official ACC gain while approaching epoch120 GT5 retention and reducing FP/FN through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 120, retained Top-K metadata, and that `results.csv` plus 121 JSON files contain no numeric NaN/Inf values. Scoped text-artifact scans found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 125 official-val monitor
...
A read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17928 / 24564 MiB`; short utilization samples were low/bursty (`2%, 0%, 0%, 0%, 18%`, final sample `0%`), but `last.pt` was still updating and the training process was alive. This is not a reason to interrupt or alter `batch=32` mid-run.

`results.csv` has an ordinary-validation row labeled epoch `126`:

```text
val/f1: 0.956956
val/precision: 0.956221
val/recall: 0.957692
val/fp: 57
val/fn: 55
val/decode/k5_to_output4_rate: 0.138889
```
...
`official_best_summary.json` at the run root has official-val candidates through epoch `125`. The current official-best row remains epoch `115`:
...
The latest official-val candidate is epoch `125`:

```text
official_acc: 0.957193
official_fp: 0.048026
official_fn: 0.033287
count_acc_3/4/5: 0.923767 / 0.909091 / 0.783784
gt5_output5_rate: 0.783784
gt5_count_head_under_rate: 0.121622
gt5_valid_points_fail_rate: 0.094595
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.140845
rescue_precision: 0.770492
matched/unmatched quality mean: 0.903824 / 0.808772
```
...
Epoch `115` still exceeds the current-code K32 audit (`0.953756`) by `+0.004204`, countboundary (`0.954137`) by `+0.003823`, old FT6 (`0.954782`) by `+0.003178`, prior K56 epoch83 best (`0.956905`) by `+0.001055`, and epoch109 (`0.957664`) by `+0.000296`. It remains below legacy `0.959224` by `-0.001264` and below the 0.97 objective. Epoch125 is slightly above epoch120 by `+0.000077` but remains below epoch115 by `-0.000767`.

Alternatives considered:

- Promote epoch 115 because it remains the official-best.
- Stop at epoch 126 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 115.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `126/180`, and official-val still has Top-K evidence after epoch 120. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch-115 official ACC gain while approaching epoch121/122 GT5 retention and reducing FP/FN through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 125, retained Top-K metadata, and that `results.csv` plus 126 JSON files contain no numeric NaN/Inf values. Scoped text-artifact scans found no `--split test`, `split: test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 137 Top-K monitor
...
A read-only remote audit and a read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17.9-18.6 / 24.6 GiB`. Utilization samples were bursty (`18%, 39%, 86%, 18%, 52%`, with a later `0%` sample during an interval), and `results.csv`, `last.pt`, and official summaries were still updating. This is not a reason to interrupt or alter `batch=32` mid-run.

`results.csv` has an ordinary-validation row labeled epoch `138`:

```text
val/f1: 0.959020
val/precision: 0.954996
val/recall: 0.963077
val/fp: 59
val/fn: 48
val/decode/k5_to_output4_rate: 0.094595
...
`official_best_summary.json` at the run root has official-val candidates through epoch `137`. The current official-best row remains epoch `127`:

```text
best_epoch: 127
official_acc: 0.958484
official_fp: 0.047383
official_fn: 0.029614
official_score: 0.956944
count_acc_3/4/5: 0.928251 / 0.893939 / 0.810811
gt5_output5_rate: 0.810811
gt5_count_head_under_rate: 0.067568
gt5_valid_points_fail_rate: 0.121622
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.013514
decode/k5_to_output4_rate: 0.133333
rescue_precision: 0.753846
rate_4_to_5: 0.075758
rate_5_to_4: 0.189189
matched/unmatched quality mean: 0.903372 / 0.813948
```

The latest official-val candidate is epoch `137`:

```text
official_acc: 0.958429
official_fp: 0.048577
official_fn: 0.031680
count_acc_3/4/5: 0.923767 / 0.878788 / 0.770270
gt5_output5_rate: 0.770270
gt5_count_head_under_rate: 0.094595
gt5_valid_points_fail_rate: 0.135135
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.013514
decode/k5_to_output4_rate: 0.162162
rescue_precision: 0.774194
matched/unmatched quality mean: 0.909715 / 0.820663
```
...
Epoch `127` refreshes K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.004728`, countboundary (`0.954137`) by `+0.004347`, old FT6 (`0.954782`) by `+0.003702`, prior K56 epoch83 best (`0.956905`) by `+0.001579`, and prior epoch115 best (`0.957960`) by `+0.000524`. It remains below legacy `0.959224` by `-0.000740` and below the 0.97 objective.

Epoch `128` is nearly tied with epoch `127`:

```text
official_acc: 0.958443
gt5_output5_rate: 0.837838
gt5_valid_points_fail_rate: 0.108108
```

This makes epoch127/128 useful paired evidence: epoch127 has the best official ACC and FN, while epoch128 shows the GT5-retention side of the remaining tradeoff. Epoch137 entered Top-K third, but regressed GT5 output and valid-points survival versus epoch128.

Alternatives considered:

- Promote epoch 127 because it is the new official-best.
- Stop at epoch 138 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 127.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `138/180`, and official-val is still retaining multiple Top-K candidates near legacy. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below legacy, the next controlled candidate should preserve epoch127 FP/FN gains while approaching epoch128 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 137, retained Top-K metadata, and that `results.csv` has 138 rows with no numeric NaN/Inf values. The run has 138 JSON files with no parse error and no numeric NaN/Inf values. A scoped text-artifact scan of 277 files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 145 near-legacy monitor
...
A read-only remote audit and a read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory was about `17.9-18.6 / 24.6 GiB`. Utilization remains bursty, but process state, checkpoint metadata, and `results.csv` updates confirm the run is healthy.

`results.csv` has an ordinary-validation row labeled epoch `146`:

```text
val/f1: 0.958621
val/precision: 0.954962
val/recall: 0.962308
val/fp: 59
val/fn: 49
val/decode/k5_to_output4_rate: 0.081081
```

The best ordinary-validation row so far by `val/f1` is now epoch `142`:

```text
val/f1: 0.962083
val/precision: 0.958047
val/recall: 0.966154
val/fp: 55
val/fn: 44
```

`official_best_summary.json` at the run root has official-val candidates through epoch `145`. The current official-best row is epoch `140`:

```text
best_epoch: 140
official_acc: 0.959200
official_fp: 0.045455
official_fn: 0.027319
official_score: 0.957745
count_acc_3/4/5: 0.919283 / 0.863636 / 0.797297
gt5_output5_rate: 0.797297
gt5_count_head_under_rate: 0.108108
gt5_valid_points_fail_rate: 0.094595
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.109589
rescue_precision: 0.769231
rate_3_to_4: 0.080717
rate_4_to_5: 0.090909
rate_5_to_4: 0.202703
matched/unmatched quality mean: 0.909307 / 0.824564
```

The latest official-val candidate is epoch `145`:

```text
official_acc: 0.958931
official_fp: 0.046373
official_fn: 0.028237
count_acc_3/4/5: 0.919283 / 0.863636 / 0.837838
gt5_output5_rate: 0.837838
gt5_count_head_under_rate: 0.067568
gt5_valid_points_fail_rate: 0.094595
gt5_candidate_pool_shortfall_rate: 0.000000
...
rescue_precision: 0.764706
matched/unmatched quality mean: 0.910776 / 0.826427
```
...
Epoch `140` refreshes K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.005444`, countboundary (`0.954137`) by `+0.005063`, old FT6 (`0.954782`) by `+0.004418`, prior K56 epoch127 best (`0.958484`) by `+0.000716`, and prior epoch115 best (`0.957960`) by `+0.001240`. It remains below legacy `0.959224` by `-0.000024` and below the 0.97 objective.

Epoch `145` is close enough to epoch `140` to be diagnostically useful while improving GT5 retention:
...
This makes epoch140/145 useful paired evidence: epoch140 has the best official ACC and lower FP/FN, while epoch145 shows the stronger GT5-retention side of the remaining tradeoff. The run is very close to the legacy reference but has not exceeded it, and it is not test-ready.

Alternatives considered:

- Promote epoch 140 because it nearly matches legacy.
- Stop at epoch 146 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 140.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `146/180`, and official-val is still retaining multiple Top-K candidates near legacy. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below or just around legacy, the next controlled candidate should preserve epoch140 FP/FN gains while approaching epoch145 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 145, retained Top-K metadata, and that `results.csv` has 146 rows with no numeric NaN/Inf values. The run has 147 JSON files with no parse error and no numeric NaN/Inf values. A scoped text-artifact scan of 295 files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 151 monitor
...
A read-only remote audit and a read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory remained around `17.9-18.6 / 24.6 GiB`. Utilization samples were bursty, including high-utilization samples, while process state, checkpoint metadata, and `results.csv` updates confirmed the run is healthy.

`results.csv` has an ordinary-validation row labeled epoch `151`:

```text
val/f1: 0.959051
val/precision: 0.954303
val/recall: 0.963846
val/fp: 60
val/fn: 47
val/decode/k5_to_output4_rate: 0.052632
```

The best ordinary-validation row so far by `val/f1` remains epoch `142`:
...
`official_best_summary.json` at the run root has official-val candidates through epoch `151`. The current official-best row remains epoch `140`:
...
The latest official-val candidate is epoch `151`:

```text
official_acc: 0.958788
official_fp: 0.045592
official_fn: 0.030073
official_score: 0.957274
count_acc_3/4/5: 0.932735 / 0.878788 / 0.837838
gt5_output5_rate: 0.837838
gt5_count_head_under_rate: 0.081081
gt5_valid_points_fail_rate: 0.081081
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.106667
rescue_precision: 0.776119
rate_3_to_4: 0.067265
rate_4_to_5: 0.075758
rate_5_to_4: 0.162162
matched/unmatched quality mean: 0.914003 / 0.833557
```
...
Epoch `140` remains K56 official-best and exceeds the current-code K32 audit (`0.953756`) by `+0.005444`, countboundary (`0.954137`) by `+0.005063`, old FT6 (`0.954782`) by `+0.004418`, prior K56 epoch127 best (`0.958484`) by `+0.000716`, and prior epoch115 best (`0.957960`) by `+0.001240`. It remains below legacy `0.959224` by `-0.000024` and below the 0.97 objective.

Epoch `149`/`151` are diagnostically useful because they improve GT5 retention:

```text
epoch149 official_acc: 0.959112
epoch151 official_acc: 0.958788
epoch151 gt5_output5_rate: 0.837838
epoch151 gt5_count_head_under_rate: 0.081081
epoch151 gt5_valid_points_fail_rate: 0.081081
```

This keeps the same tradeoff: epoch140 has the best official ACC and lower FP/FN, while later epochs show stronger GT5 retention at lower official ACC. The run is very close to the legacy reference but has not exceeded it, and it is not test-ready.

Alternatives considered:

- Promote epoch 140 because it nearly matches legacy.
- Stop at epoch 151 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 140.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, only at ordinary epoch `151/180`, and official-val is still retaining multiple Top-K candidates near legacy. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a clearer plateau below or just around legacy, the next controlled candidate should preserve epoch140 FP/FN gains while approaching epoch149/151 GT5 retention through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 151, retained Top-K metadata, and that `results.csv` has 151 rows with no numeric NaN/Inf values. The run has 152 JSON files with no parse error and no numeric NaN/Inf values. A scoped text-artifact scan of 305 files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 156 monitor
...
A main-thread read-only audit and a read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory remained around `17.9-18.6 / 24.6 GiB`. Process state, checkpoint metadata, and `results.csv` updates confirm the run is healthy.

`results.csv` has an ordinary-validation row labeled epoch `156`:

```text
val/f1: 0.957121
val/precision: 0.952744
val/recall: 0.961538
val/fp: 62
val/fn: 50
```
...
`official_best_summary.json` at the run root has official-val candidates through epoch `155`. The current official-best row is epoch `152`:
...
The latest official-val candidate in the summary is epoch `155`:

```text
official_acc: 0.959165
official_fp: 0.045225
official_fn: 0.028466
official_score: 0.957691
count_acc_3/4/5: 0.928251 / 0.878788 / 0.851351
gt5_output5_rate: 0.851351
gt5_count_head_under_rate: 0.067568
gt5_valid_points_fail_rate: 0.081081
...
decode/k5_to_output4_rate: 0.105263
rescue_precision: 0.779412
matched/unmatched quality mean: 0.915389 / 0.829516
```
...
Epoch `152` is the first K56 official-val candidate in this run to exceed the legacy reference `0.959224`:
...
This is an important K56 milestone, but not a promotion decision. The run is only at ordinary epoch `156/180`, the margin over legacy is small, and the 0.97 objective remains unmet. The stronger K56 row improves GT5 output/count5 retention (`gt5_output5_rate=0.851351`, `count_acc_5=0.851351`) while keeping GT5 candidate shortfall and GT5 NMS at zero. The remaining bottleneck is GT5 `5->4`, GT3/GT4 over-output, and valid-points/quality separation versus FP.

Alternatives considered:

- Promote epoch 152 immediately because it exceeds legacy on official-val.
- Stop at epoch 156 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 152.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use ordinary `best.pt` or test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.

Tradeoff:

Continuing uses server time, but the run is healthy, official-val is still updating retained Top-K candidates, and stopping now would leave the first clean K56 baseline incomplete. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a stable plateau after completion, the next controlled candidate should preserve epoch152 FP/FN gains while reducing GT5 `5->4` and false fifth-lane pressure through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 155, retained Top-K metadata, and that `results.csv` has 156 rows with no numeric NaN/Inf values. The run has 156 JSON files with no parse error and no numeric NaN/Inf values. A scoped text-artifact scan of 313 files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.

Mainline or experiment:

Experimental baseline monitoring decision. K56 is still not promoted over K32, and no official-test claim is available.

## 2026-06-14: Continue Q12-K56 b32 baseline after epoch 160 monitor
...
A main-thread read-only audit and a read-only `gcs_experiment_analyst` subagent audit on `2026-06-14` found the process alive with the intended formal K56 command: `--batch 32`, `--workers 4`, `--imgsz 544 960`, K56 data/model, `--gcs-official-best`, and `--gcs-official-best-top-k 5`. GPU memory remained around `17.9 / 24.6 GiB`. Process state, checkpoint metadata, and `results.csv` updates confirm the run is healthy.

`results.csv` has an ordinary-validation row labeled epoch `160`:

```text
val/f1: 0.957822
val/precision: 0.954893
val/recall: 0.960769
val/fp: 59
val/fn: 51
val/decode/k5_to_output4_rate: 0.105263
```
...
`official_best_summary.json` at the run root has official-val candidates through epoch `160`. The current official-best row remains epoch `152`:
...
The latest official-val candidate is epoch `160`:

```text
official_acc: 0.959138
official_fp: 0.043618
official_fn: 0.027548
official_score: 0.957575
count_acc_3/4/5: 0.932735 / 0.878788 / 0.851351
gt5_output5_rate: 0.851351
gt5_count_head_under_rate: 0.067568
gt5_valid_points_fail_rate: 0.081081
gt5_candidate_pool_shortfall_rate: 0.000000
gt5_top5_suppressed_by_nms_rate: 0.000000
decode/k5_to_output4_rate: 0.093333
rescue_precision: 0.808824
matched/unmatched quality mean: 0.917138 / 0.836530
```
...
Epoch `152` still exceeds the legacy reference `0.959224`:
...
This remains an important K56 milestone, but not a promotion decision. The run is only at ordinary epoch `160/180`, the margin over legacy is small, and the 0.97 objective remains unmet. Candidate pool and GT5 NMS are not the active blocker. The remaining bottleneck is GT5 `5->4`, GT3/GT4 over-output, and valid-points/quality separation versus FP.

Alternatives considered:

- Promote epoch 152 because it remains best and exceeds legacy on official-val.
- Stop at epoch 160 and run final test.
- Start K56 Count/Quality calibration immediately from epoch 152.
- Increase batch size in the middle of the in-progress run to use more GPU memory.
- Use ordinary `best.pt` or test to choose between Top-K checkpoints.
- Continue the healthy baseline while official-val remains active.
...
Continuing uses server time, but the run is healthy, official-val is still being updated, and stopping now would leave the first clean K56 baseline incomplete. Increasing batch size mid-run would break comparability and require restarting the formal baseline. Launching a competing calibration now would consume the same 24GB GPU before the first clean K56 baseline has matured. If this run later reaches a stable plateau after completion, the next controlled candidate should preserve epoch152 FP/FN gains while reducing GT5 `5->4` and false fifth-lane pressure through training-side Count/Quality calibration from an official-val-selected K56 checkpoint.

Validation evidence:

Read-only checks confirmed the process list, GPU memory/utilization samples, command line, `results.csv`, root `official_best_summary.json`, official-val candidates through epoch 160, retained Top-K metadata, and that `results.csv` has 160 rows with no numeric NaN/Inf values. The run has 160 JSON files with no parse error and no numeric NaN/Inf values. A scoped text-artifact scan of 321 files found no `--split test`, `split: test`, `split=test`, `test_label.json`, or `test_set` hits, and no `RuntimeError`, `shape error`, `shape mismatch`, or `Traceback` hits. `args.yaml` records `split=val`, `gcs_official_best_split=val`, `imgsz=[544, 960]`, `gcs_imgsz=[544, 960]`, and K56 data/model. No test evidence was used.
