...
- A follow-up post-fix `gt5segq_unmatched` attempt remained non-promotable (`official_best` around `0.953075`) and pushed GT5 output down, so do not continue the strong hard-negative/segment recipe as the next main path. The next server run should include a clean FT6 control, a GT3/GT4/GT5 joint Count calibration candidate, and only a mild unmatched-only segment-quality candidate.
- 2026-06-13 code review found a structural mismatch left after visible-segment decode rank: `CandidateAwareCountHead` still used all-anchor `valid_mean` for lane quality, top-query selection, and top4/top5 cardinality features. That suppressed short 5-7 point edge lanes before Count Head K selection. The code now aligns Count Head evidence with decode using `exist * visible_segment_mean_valid * min(visible_segment_points / 12, 1)`, while retaining all-anchor mean as an auxiliary feature. This is a root implementation fix but has no official-val improvement claim until the next server run completes.
