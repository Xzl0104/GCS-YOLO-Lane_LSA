...
Only the explicit TuSimple contract was changed from the legacy `Q=8/K=32/fixed_y=[0.98,0.25]` setup to the current `Q=12/K=56/fixed_y=710/720 -> 160/720` setup, plus the branch-local default-off `count_boundary_loss`, train-only `gcs_hard_sampling`, default-off E3-lite `gcs_spurious_neg`, training-time `official_best` protocol additions, the default-off `valid_before_maxdet` query decode option, the 2026-07-06 user-requested default-off query Count Head ablation, and the 2026-07-28 user-reopened default-off Q12/env30 gated candidate v2 path. The 5-25-3 algorithm body is intentionally not upgraded to the later mainline Count Head, Quality Head, Survival Head, near-miss, or unrelated post-env30 experiment machinery.

As of 2026-07-28, active code/config is restored to the env30 baseline commit `86c8fb31c`. All commits after that anchor are rejected experiment records unless a future task explicitly reopens one with new official-val/train-side gates. This includes env30 staticref/valid-neg/near20 follow-ups, Q20/Q24 protected-static or dual-head routes, Q12 dual-head, query extent, short local-refine, and the rejected 2026-07-27 lateral/gated candidate mechanisms. The 2026-07-28 gated candidate v2 implementation is the explicit user-reopened default-off exception and must pass fresh gates before any TEST use.
