...
Only the explicit TuSimple contract was changed from the legacy `Q=8/K=32/fixed_y=[0.98,0.25]` setup to the current `Q=12/K=56/fixed_y=710/720 -> 160/720` setup, plus the branch-local default-off `count_boundary_loss`, train-only `gcs_hard_sampling`, default-off E3-lite `gcs_spurious_neg`, training-time `official_best` protocol additions, the default-off `valid_before_maxdet` query decode option, the 2026-07-06 user-requested default-off query Count Head ablation, and the env30-family default-off `gcs_short_geom` / `gcs_boundary_pseudo_neg` protocol present at `86c8fb31c`. The 5-25-3 algorithm body is intentionally not upgraded to the later mainline Count Head, Quality Head, Survival Head, near-miss, post-`86c8fb31c` tiered rescue, Q12 ultrashort dataref, final-query extra guard/gate, router/selector, far-spurious, or count-contract/ranking machinery.

Active source/config is the rollback state at commit `86c8fb31c` (`Add GT4 GT5 weak geometry rescue run`). Previous q12-k56 experiment documentation remains historical context. Do not delete it, and do not read post-`86c8fb31c` sections as active algorithm behavior; those later experiments are rejected legacy records unless a future task explicitly reopens them.
