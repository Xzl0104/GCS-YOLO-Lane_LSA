...
from ultralytics.utils.gcs_shape import assert_gcs_image_tensor
...
class BaseModel(torch.nn.Module):
    ...
    def _arg_value(args, name: str):
        """Read one optional argument from a dict or namespace."""
        if isinstance(args, dict):
            return args.get(name)
        return getattr(args, name, None)

    def _gcs_shape_contract(self):
        """Return the configured GCS H,W contract when this is a GCS lane model."""
        for value in (
            getattr(self, "gcs_imgsz", None),
            self._arg_value(getattr(self, "args", None), "gcs_imgsz"),
            self._arg_value(getattr(self, "args", None), "image_shape"),
        ):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor): The last output of the model.
        """
        orig_size = tuple(x.shape[-2:]) if isinstance(x, torch.Tensor) and x.ndim >= 4 else None
        has_gcs_head = getattr(self, "task", None) == "gcs_lane" or any(isinstance(m, GCSLaneHead) for m in self.model)
        if has_gcs_head:
            gcs_imgsz = self._gcs_shape_contract()
            assert gcs_imgsz is not None, (
                f"{self.__class__.__name__}.forward requires model.gcs_imgsz or model.args.gcs_imgsz. "
                "Do not let GCS model forward infer shape from scalar args.imgsz."
            )
            assert_gcs_image_tensor(x, gcs_imgsz, name="model input", context=f"{self.__class__.__name__}.forward")
        ...
        for m in self.model:
            ...
            if profile:
                self._profile_one_layer(m, x, dt, orig_size=orig_size)
            x = m(x, orig_size=orig_size) if isinstance(m, GCSLaneHead) else m(x)  # run
    ...
    def _profile_one_layer(self, m, x, dt, orig_size=None):
        ...
        profile_input = x.copy() if c else x
        flops = (
            m.profile_flops(profile_input)
            if isinstance(m, GCSLaneHead)
            else thop.profile(m, inputs=[profile_input], verbose=False)[0] / 1e9 * 2
            if thop
            else 0
        )  # GFLOPs
        t = time_sync()
        for _ in range(10):
            profile_input = x.copy() if c else x
            m(profile_input, orig_size=orig_size) if isinstance(m, GCSLaneHead) else m(profile_input)
...
def load_checkpoint(weight, device=None, inplace=True, fuse=False):
    ...
    _assert_current_gcs_checkpoint_model(model, weight)
...
def _assert_current_gcs_checkpoint_model(model: torch.nn.Module, weight) -> None:
    """Reject obsolete pickled GCS checkpoints instead of silently running old lane-head code."""
    if not any(isinstance(m, GCSLaneHead) for m in model.modules()):
        return
    required = (
        "point_embed",
        "point_coord_mlp",
...
def _resolve_activation_from_yaml(act):
    """Resolve a YAML activation string without evaluating arbitrary code."""
    if isinstance(act, torch.nn.Module):
        return act
    if not isinstance(act, str):
        raise TypeError(f"activation must be a torch.nn module or string, got {type(act).__name__}.")
    ...
    expr = ast.parse(act.strip(), mode="eval").body
    ...
    if isinstance(expr, ast.Call):
        ...
        for kw in expr.keywords:
            if kw.arg is None:
                raise ValueError(f"Unsupported activation expression: {act}")
            kwargs[kw.arg] = ast.literal_eval(kw.value)
    else:
        func, args, kwargs = expr, [], {}
    ...
    if len(parts) >= 2 and parts[-2:] == ["nn", parts[-1]]:
        name = parts[-1]
    elif len(parts) == 1:
        name = parts[0]
    else:
    ...
    if name not in allowed or not hasattr(torch.nn, name):
        raise ValueError(f"Unsupported activation '{name}'. Allowed activations: {sorted(allowed)}")
...
def parse_model(d, ch, verbose=True):
    ...
    if act:
        Conv.default_act = _resolve_activation_from_yaml(act)  # redefine default activation, e.g. torch.nn.SiLU()
        if verbose:
...
def guess_model_scale(model_path):
    ...
    match = re.search(r"yolo(e-)?[v]?\d+([nslmx])", stem)
    if match:
...
def guess_model_task(model):
    ...
    if isinstance(model, (str, Path)):
        ...
        if "-seg" in model.stem or "segment" in model.parts:
            ...
            return "detect"
        elif "gcs" in model.parts or ("gcs" in model.stem and "lane" in model.stem):
            return "gcs_lane"
