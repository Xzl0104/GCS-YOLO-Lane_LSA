...
def model_state_is_finite(model: nn.Module) -> bool:
    """Return True when all floating tensors in a model state_dict are finite."""
    return all(torch.isfinite(v).all() for v in model.state_dict().values() if isinstance(v, torch.Tensor))


def model_to_half_keep_bn_float(model: nn.Module) -> nn.Module:
    """Convert a model to FP16 while keeping BatchNorm modules in FP32.

    BatchNorm running variances can legitimately grow beyond the FP16 maximum during
    GCS lane training. Keeping BN in FP32 avoids false Inf values during checkpoint
    serialization while preserving the usual FP16 checkpoint size savings elsewhere.
    """
    for module in model.modules():
        keep_float = isinstance(module, nn.modules.batchnorm._BatchNorm)
        for param in module.parameters(recurse=False):
            param.data = param.data.float() if keep_float else param.data.half()
            if param.grad is not None:
                param.grad.data = param.grad.data.float() if keep_float else param.grad.data.half()
        for name, buffer in module._buffers.items():
            if isinstance(buffer, torch.Tensor) and buffer.dtype.is_floating_point:
                module._buffers[name] = buffer.float() if keep_float else buffer.half()
    return model
            ...
            for k, v in self.ema.state_dict().items():
                if v.dtype.is_floating_point:  # true for FP16 and FP32
                    ...
                    if not torch.isfinite(src).all().item():
                        if self.finite_warning_count < 3:
                            LOGGER.warning(f"Skipping EMA update for non-finite model tensor: {k}")
                        self.finite_warning_count += 1
                        continue
                    src = src.to(dtype=v.dtype)
                    if not torch.isfinite(v).all().item():
                        if self.finite_warning_count < 3:
                            LOGGER.warning(f"Repairing non-finite EMA tensor from current model tensor: {k}")
                        self.finite_warning_count += 1
                        v.copy_(src)
                    else:
                        v *= d
...
def strip_optimizer(f: str | Path = "best.pt", s: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    model_to_half_keep_bn_float(x["model"])  # to FP16 while keeping BatchNorm stats finite
