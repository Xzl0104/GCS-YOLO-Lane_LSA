...
SWEEP_SELECTION_KEYS = (
    ...
    {"key": "extent_decode_priority", "direction": "min"},
...
def extent_decode_priority(mode: Any) -> int:
    """Return the official sweep tie-break priority for query extent decode modes."""
    normalized = str(mode or "none").strip().lower()
    if normalized not in EXTENT_DECODE_MODE_PRIORITY:
        raise ValueError(f"Unsupported extent_decode_mode={mode!r}.")
    return int(EXTENT_DECODE_MODE_PRIORITY[normalized])
...
def selection_sort_key(row: dict[str, Any], ordered_keys: tuple[dict[str, str], ...]) -> tuple:
    ...
    for item in ordered_keys:
        ...
        if key == "extent_decode_priority":
            value = row.get(key, extent_decode_priority(row.get("extent_decode_mode", "none")))
        else:
            value = row.get(key, 0)
        if direction == "max":
