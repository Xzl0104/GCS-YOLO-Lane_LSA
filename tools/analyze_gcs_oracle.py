...
from tools.eval_gcs import gated_assignment, label_path_for_image, load_gcs_label, match_lanes, pair_geometry
from tools.infer_gcs import collect_images, load_gcs_model, preprocess_image
from ultralytics.utils.gcs_postprocess import decode_gcs_predictions, sort_lane_bottom_to_top
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--nms-dist-px", type=float, default=50.0, help="Baseline Lane-NMS threshold. 0 disables.")
    parser.add_argument("--max-det", type=int, default=8, help="Maximum decoded baseline lanes.")
...
def main() -> None:
    ...
    for image_path in images:
        ...
        count_indices = np.argsort(-scores)[:topk].astype(np.int64) if topk > 0 else np.zeros((0,), dtype=np.int64)
        count_lanes = lanes_from_indices(sorted_points, scores, count_indices, img.shape[:2])
