...
Monitoring snapshot at `2026-06-14` after official-val epoch 125: the run is alive with ordinary validation through epoch 126/180; official-val sweeps are recorded through epoch 125. GPU memory is about 17.9-18.6 GiB of 24.6 GiB, so the current `batch=32` server run should not be interrupted or changed mid-run. Official-best remains epoch 115 with `official_acc=0.957960`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, old FT6 `0.954782`, prior K56 epoch83 best `0.956905`, and epoch109 `0.957664`, but still below the legacy `0.959224` target by `0.001264`. Epoch 97 remains the best ordinary-val row so far (`val/f1=0.959724`), while epochs 121/122 improved GT5 official-val retention without beating epoch115 ACC. Retained official Top-K is `115, 114, 109, 121, 122`; K56 is not promoted and no test result is claimed. Treat this as active baseline monitoring only; continue the healthy `batch=32` run and do not launch Count/Quality calibration until plateau is clearer or the run completes.
