...
Monitoring snapshot at `2026-06-14` after official-val epoch 107: the run is alive with ordinary validation through epoch 108/180; official-val sweeps are recorded through epoch 107. GPU memory is about 17.9-18.6 GiB of 24.6 GiB, so the current `batch=32` server run is not obviously underusing RTX 4090 VRAM and should not be interrupted. Official-best remains epoch 83 with `official_acc=0.956905`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 97 remains the best ordinary-val row so far (`val/f1=0.959724`), but official-val remains the selection signal. Retained official Top-K is `83, 107, 97, 101, 95`; K56 is not promoted and no test result is claimed. Epoch 107 entered Top-K second with `official_acc=0.956640`, close to epoch 83 but with worse GT5 output and valid-points survival. Treat this as active baseline monitoring only; continue the healthy `batch=32` run and do not launch Count/Quality calibration until plateau is clearer or the run completes.
