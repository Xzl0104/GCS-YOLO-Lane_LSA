...
Monitoring snapshot at `2026-06-14 11:20 CST`: the run is alive with ordinary validation through epoch 71/180 and about 17.9 GiB of 24.6 GiB GPU memory in use; official-val sweeps are recorded through epoch 70. Official-best remains epoch 61 with `official_acc=0.954807`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 67 is the best ordinary-val row so far (`val/f1=0.958157`), but ordinary validation is not the selection signal. Epoch 70 official-val is `0.952543`, and retained official Top-K is `61, 69, 68, 52, 67`; K56 is not promoted and no test result is claimed. Keep the healthy `batch=32` run alive; use a throughput-only batch probe before launching a new formal run if more of the 24GB server GPU should be used.
