...
Monitoring snapshot at `2026-06-14 12:45 CST`: the run is alive with ordinary validation through epoch 95/180; official-val sweeps are recorded through epoch 94. GPU memory is about 18.6 GiB of 24.6 GiB during active training, with utilization samples reaching 100%, so the current `batch=32` server run is using the RTX 4090 normally and should not be interrupted. Official-best remains epoch 83 with `official_acc=0.956905`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 73 remains the best ordinary-val row so far (`val/f1=0.959693`), but official-val remains the selection signal. Retained official Top-K is `83, 85, 84, 87, 94`; K56 is not promoted and no test result is claimed. Epoch 94 entered Top-K with `official_acc=0.955354` and `gt5_valid_points_fail_rate=0.094595`, but it did not beat epoch 83 and has higher FP/FN. Treat this as short-term official-val plateau only; continue the healthy `batch=32` run and do not launch Count/Quality calibration until plateau is clearer or the run completes.
