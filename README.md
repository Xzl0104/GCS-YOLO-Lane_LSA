...
Monitoring snapshot at `2026-06-14 11:05 CST`: the run is alive with ordinary validation through epoch 66/180 and about 17.5-18.2 GiB GPU memory in use; official-val sweeps are recorded through epoch 65. Official-best remains epoch 61 with `official_acc=0.954807`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 65 is the best ordinary-val row so far (`val/f1=0.957390`), but its official-val row is only `0.952533`, so K56 is not promoted and no test result is claimed.
