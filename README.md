...
Monitoring snapshot at `2026-06-14` after official-val epoch 131: the run is alive with ordinary validation through epoch 132/180; official-val sweeps are recorded through epoch 131. GPU memory is about 17.9 GiB of 24.6 GiB, so the current `batch=32` server run should not be interrupted or changed mid-run. Official-best is now epoch 127 with `official_acc=0.958484`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, old FT6 `0.954782`, prior K56 epoch83 best `0.956905`, and prior epoch115 best `0.957960`, but still below the legacy `0.959224` target by `0.000740`. Epoch 97 remains the best ordinary-val row so far (`val/f1=0.959724`), while epoch128 is nearly tied with epoch127 and has better GT5 retention. Retained official Top-K is `127, 128, 129, 131, 126`; K56 is not promoted and no test result is claimed. Treat this as active baseline monitoring only; continue the healthy `batch=32` run and do not launch Count/Quality calibration until plateau is clearer or the run completes.
