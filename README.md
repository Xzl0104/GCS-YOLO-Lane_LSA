...
Monitoring snapshot at `2026-06-14 12:19 CST`: the run is alive with ordinary validation through epoch 88/180 and about 17.9 GiB of 24.6 GiB GPU memory in use; official-val sweeps are recorded through epoch 87. Official-best is now epoch 83 with `official_acc=0.956905`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 73 remains the best ordinary-val row so far (`val/f1=0.959693`), but official-val remains the selection signal. Retained official Top-K is `83, 85, 84, 87, 82`; K56 is not promoted and no test result is claimed. Epoch 83 improves FP/FN over epoch 82 and raises official-val Accuracy by `+0.001613`, but GT5 output is lower than epoch 82 (`gt5_output5_rate=0.810811`) and `gt5_valid_points_fail_rate=0.108108` is still slightly above the diagnostic `0.1` line, so the baseline has not clearly plateaued and is not final-test-ready. Keep the healthy `batch=32` run alive; use a throughput-only batch probe before launching a new formal run if more of the 24GB server GPU should be used.
