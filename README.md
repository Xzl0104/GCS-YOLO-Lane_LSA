...
Monitoring snapshot at `2026-06-14 11:51 CST`: the run is alive with ordinary validation through epoch 79/180 and about 17.9 GiB of 24.6 GiB GPU memory in use; official-val sweeps are recorded through epoch 79. Official-best remains epoch 73 with `official_acc=0.955238`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 73 is also the best ordinary-val row so far (`val/f1=0.959693`), but official-val remains the selection signal. Retained official Top-K is `73, 76, 61, 72, 79`; K56 is not promoted and no test result is claimed. Epoch 76 came close to best and epoch 79 entered Top-K, so the baseline has not clearly plateaued. Keep the healthy `batch=32` run alive; use a throughput-only batch probe before launching a new formal run if more of the 24GB server GPU should be used.
