...
Monitoring snapshot at `2026-06-14 12:06 CST`: the run is alive with ordinary validation through epoch 83/180 and about 17.9 GiB of 24.6 GiB GPU memory in use; official-val sweeps are recorded through epoch 82. Official-best is now epoch 82 with `official_acc=0.955292`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, and old FT6 `0.954782` references, but still below the legacy `0.959224` target. Epoch 73 remains the best ordinary-val row so far (`val/f1=0.959693`), but official-val remains the selection signal. Retained official Top-K is `82, 73, 76, 61, 80`; K56 is not promoted and no test result is claimed. Epoch 82 is only a tiny official-val gain over epoch 73, but it improves GT5 survival (`gt5_output5_rate=0.891892`, `gt5_valid_points_fail_rate=0.067568`), so the baseline has not clearly plateaued. Keep the healthy `batch=32` run alive; use a throughput-only batch probe before launching a new formal run if more of the 24GB server GPU should be used.
