...
Monitoring snapshot at `2026-06-14` after official-val epoch 151: the run is alive with ordinary validation through epoch 151/180; official-val sweeps are recorded through epoch 151. GPU memory is about 17.9-18.6 GiB of 24.6 GiB, so the current `batch=32` server run should not be interrupted or changed mid-run. Official-best is epoch 140 with `official_acc=0.959200`, above the current-code K32 audit `0.953756`, countboundary `0.954137`, old FT6 `0.954782`, prior K56 epoch127 best `0.958484`, and prior epoch115 best `0.957960`, but still below the legacy `0.959224` target by `0.000024`. Epoch 142 remains the best ordinary-val row so far (`val/f1=0.962083`), while epoch151 is the latest official-val candidate at `0.958788` with stronger GT5 retention than epoch140. Retained official Top-K is `140, 141, 143, 149, 146`; K56 is not promoted and no test result is claimed. Treat this as active baseline monitoring only; continue the healthy `batch=32` run and do not launch Count/Quality calibration until plateau is clearer or the run completes.
